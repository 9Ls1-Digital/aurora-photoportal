...
$onboarding_bg_desktop = !empty($branding['photographer_login_bg_desktop']) ? $branding['photographer_login_bg_desktop'] : NLS1_FOTOPORTAL_PLUGIN_URL . 'assets/aurora-login-background.png';
$onboarding_bg_mobile = !empty($branding['photographer_login_bg_mobile']) ? $branding['photographer_login_bg_mobile'] : $onboarding_bg_desktop;
...
$gallery_notifications = !empty($enabled['favorites_comments']) ? NLS1_Fotoportal_Admin::gallery_activity_notifications((int)$account->id) : [];
...
$has_customer_portal = !empty($enabled['customer_portal']);
$has_favorites_comments = !empty($enabled['favorites_comments']);
$has_premium_proof = !empty($enabled['premium_proof']);
$has_hq_delivery = !empty($enabled['hq_delivery']);
...
$view_titles = [
    ...
    'hq_delivery' => ['Digital levering', 'Sikker levering av ferdige høyoppløselige bilder.'],
...
$is_trial_account = (($account->status ?? '') === 'trial' || ($account->plan_name ?? '') === 'Trial');
$is_demo_journey_account = class_exists('NLS1_Aurora_Account_Platform') && NLS1_Aurora_Account_Platform::demo_journey_enabled((int)$account->id);
$dashboard_projects = NLS1_Fotoportal_Admin::get_projects($is_trial_account);
$dashboard_contracts = NLS1_Fotoportal_Admin::get_contracts($is_trial_account);
$dashboard_galleries = NLS1_Fotoportal_Admin::get_galleries(0, $is_trial_account);
...
$demo_pack_state = class_exists('NLS1_Aurora_Demo_Content') ? NLS1_Aurora_Demo_Content::account_state((int)$account->id) : [];
$seed_demo_project_id = (int)($demo_pack_state['demo_project_id'] ?? 0);
$seed_demo_client_id = (int)($demo_pack_state['demo_client_id'] ?? 0);
// The guided trial must follow the photographer's own training customer/project, not the
// pre-provisioned Aurora example project. Otherwise seeded demo images make later steps
// look completed before the photographer has actually performed them.
$training_started = !empty($demo_pack_state['kit_downloaded_at']) ? strtotime((string)$demo_pack_state['kit_downloaded_at']) : 0;
$training_projects = array_values(array_filter(NLS1_Fotoportal_Admin::get_projects(false), function($p) use ($training_started){ return !$training_started || (!empty($p->created_at) && strtotime((string)$p->created_at) >= $training_started); }));
$demo_project = !empty($training_projects) ? reset($training_projects) : null;
$demo_project_id = $demo_project ? (int)$demo_project->id : 0;
$training_clients = array_values(array_filter(NLS1_Fotoportal_Admin::get_clients(false), function($c) use ($training_started){ return !$training_started || (!empty($c->created_at) && strtotime((string)$c->created_at) >= $training_started); }));
$demo_client_id = $demo_project ? (int)$demo_project->client_id : (!empty($training_clients) ? (int)reset($training_clients)->id : 0);
$demo_flow = $demo_project_id ? NLS1_Fotoportal_Admin::project_delivery_state($demo_project_id) : ['project'=>false,'contract_registered'=>false,'contract_signed'=>false,'documents'=>false,'gallery'=>false,'paid'=>false,'delivery_released'=>false];
$demo_customer_portal_url = $demo_client_id ? NLS1_Fotoportal_Admin::customer_portal_url($demo_client_id) : '';
$demo_kit_url = class_exists('NLS1_Aurora_Demo_Content') ? wp_nonce_url(admin_url('admin-post.php?action=aurora_demo_download_kit'),'aurora_demo_download_kit') : '';

$legacy_links = [

];
$render_project_gallery_status = function($project){
    if(!$project)return;
    $pid=(int)$project->id;
    $allowed=NLS1_Fotoportal_Admin::gallery_access_allowed($pid);
    $required=NLS1_Fotoportal_Admin::project_requires_contract($pid);
    $signed=NLS1_Fotoportal_Admin::has_signed_contract($pid);
    if($allowed){$strong='Galleri åpent';$copy=$required?($signed?'Avtale signert · kundevisning er åpen':'Kundevisning er åpen'):'Avtale er valgfri · kundevisning er åpen';}
    else{$strong='Galleri låst';$copy=$required?'Venter på signert avtale':'Kundevisning er foreløpig ikke åpnet';}
    echo '<div class="aurora-gallery-gate-summary '.($allowed?'is-unlocked':'is-locked').'"><span class="dashicons '.($allowed?'dashicons-unlock':'dashicons-lock').'"></span><div><small>GALLERISTATUS</small><strong>'.esc_html($strong).'</strong><em>'.esc_html($copy).'</em></div></div>';
};
?>
<div class="aurora-workspace <?php echo $view==='onboarding' ? 'is-onboarding' : ''; ?>"<?php if ($view==='onboarding') : ?> style="--aurora-onboarding-bg:url('<?php echo esc_url($onboarding_bg_desktop); ?>');--aurora-onboarding-bg-mobile:url('<?php echo esc_url($onboarding_bg_mobile); ?>');"<?php endif; ?>>
    <aside class="aurora-workspace-sidebar">
        ...
        <div class="aurora-workspace-account">
            <?php if (!empty($photographer_profile['logo_url'])) : ?><span class="aurora-workspace-avatar is-logo"><img src="<?php echo esc_url($photographer_profile['logo_url']); ?>" alt="<?php echo esc_attr($account->account_name); ?>"></span><?php else : ?><span class="aurora-workspace-avatar"><?php echo esc_html(strtoupper(substr($account->account_name,0,1))); ?></span><?php endif; ?>
        ...
        <nav class="aurora-workspace-menu">
            ...
            <?php if ($has_favorites_comments) : ?><a class="<?php echo $view==='selections'?'is-active':''; ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('selections')); ?>"><span class="dashicons dashicons-yes-alt"></span>Bildevalg<?php if($gallery_unread): ?><b class="aurora-menu-badge"><?php echo (int)$gallery_unread; ?></b><?php endif; ?></a><?php endif; ?>
    ...
    <main class="aurora-workspace-main">
            ...
            <div class="aurora-workspace-topactions">
                ...
                <div class="aurora-profile-menu-wrap">
                    ...
                    <div class="aurora-profile-dropdown" data-aurora-profile-dropdown hidden>
                        ...
                        <a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('settings').'#vannmerke'); ?>"><span class="dashicons dashicons-format-image"></span>Rediger vannmerke</a>
                        <a class="aurora-profile-logout" href="<?php echo esc_url(function_exists('aurora_auth_logout_url') ? aurora_auth_logout_url('fotoportal','photographer') : wp_logout_url(home_url('/fotograf/'))); ?>"><span class="dashicons dashicons-exit"></span>Logg ut</a>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="aurora-workspace-content">
            ...
            <div class="aurora-workspace-titlebar">
                ...
                <?php if ($view === 'onboarding' && !empty($_GET['activated'])) : ?><div class="aurora-workspace-alert is-success"><span class="dashicons dashicons-yes-alt"></span><div><strong>Kontoen er aktivert</strong><p>Passordet er lagret. Fullfør de seks stegene nedenfor, så er Fotoportalen klar.</p></div></div><?php endif; ?>
            <?php if ($view === 'onboarding') : ?>
                ...
                ?>
                <div class="aurora-onboarding-brand">
                    <?php if(!empty($branding['logo_url'])):?><img src="<?php echo esc_url($branding['logo_url']);?>" alt="Aurora"><?php else:?><span class="aurora-onboarding-mark">A</span><?php endif;?>
                    <strong>AURORA</strong><small>Intelligent Business Platform</small>
                </div>
            ...
            <?php elseif ($view === 'dashboard') : ?>
                <?php if ($is_demo_journey_account && class_exists('NLS1_Aurora_Demo_Journey')) NLS1_Aurora_Demo_Journey::render_dashboard((int)$account->id); ?>
                ...
                <section class="aurora-dashboard-status-grid">
                    ...
                    <?php if ($has_hq_delivery) : ?><a class="aurora-dashboard-stat is-blue" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('hq_delivery')); ?>"><span class="dashicons dashicons-download"></span><div><small>Til levering</small><strong><?php echo (int)$dashboard_to_delivery; ?></strong><em>Se leveranser →</em></div></a><?php endif; ?>
                </section>
                ...
                <div class="aurora-dashboard-main-grid">
                    ...
                    <aside class="aurora-dashboard-rightcol">
                        <?php $demo_state=class_exists('NLS1_Aurora_Demo_Content')?NLS1_Aurora_Demo_Content::account_state((int)$account->id):[]; ?>
                        <?php if($is_demo_journey_account): ?><section class="aurora-workspace-card aurora-dashboard-customer-card aurora-demo-sidecard"><span class="aurora-workspace-eyebrow">GUIDET DEMO JOURNEY</span><h2>Opplæring er aktivert</h2><p>Demo Journey er valgfri og aktivert for denne fotografkontoen. Demo-kunder, prosjekter og gallerier opprettes først når fotografen gjennomfører guiden.</p><a class="aurora-dashboard-more" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('dashboard',['demo_journey'=>1])); ?>">Fortsett / start Demo Journey →</a></section><?php else: ?>                        <section class="aurora-workspace-card aurora-dashboard-customer-card"><span class="aurora-workspace-eyebrow">KUNDEOPPLEVELSE</span><h2>Vannmerke og kundevisning</h2><p>Slik vises bildene for kundene dine.</p><div class="aurora-dashboard-watermark-preview" data-position="<?php echo esc_attr($photographer_profile['watermark_position']??'bottom_right'); ?>" style="--wm-size:<?php echo (int)($photographer_profile['watermark_size']??18); ?>%;--wm-opacity:<?php echo (100-(int)($photographer_profile['watermark_opacity']??35))/100; ?>"><img src="<?php echo esc_url($branding['watermark_preview_url']); ?>" alt="Aurora testbilde"><?php if(!empty($photographer_profile['watermark_url'])):?><img class="wm" src="<?php echo esc_url($photographer_profile['watermark_url']); ?>" alt="Vannmerke"><?php else:?><span class="wm text">DITT VANNMERKE</span><?php endif;?></div><a class="aurora-dashboard-more" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('settings').'#vannmerke'); ?>">Rediger vannmerkeinnstillinger →</a></section><?php endif; ?>
                    ...
                    <div class="aurora-customer-profile-head">
                        ...
                        <div class="aurora-customer-profile-card">
                            ...
                            <div class="aurora-customer-profile-actions">
                                ...
                                <a class="aurora-secondary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('customers',['customer_id'=>$customer_id,'edit'=>1])); ?>"><span class="dashicons dashicons-edit"></span>Rediger</a><?php if($has_customer_portal): $cpurl=NLS1_Fotoportal_Admin::photographer_customer_preview_url((int)$customer_id); ?><a class="aurora-secondary-action" href="<?php echo esc_url($cpurl); ?>" target="_blank" rel="noopener"><span class="dashicons dashicons-visibility"></span>Åpne som kunde</a><?php endif; ?>
                                <a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('new')); ?>"><span class="dashicons dashicons-plus-alt2"></span>Nytt prosjekt</a>
                            </div>
                        </div>
                    </div>
                ...
                <div class="aurora-workspace-grid">
                        <section class="aurora-workspace-card">
                            ...
                            <dl class="aurora-customer-details">
                                ...
                                <div><dt>Kundeinnlogging</dt><dd><?php if($portal_user): $login_enabled=NLS1_Fotoportal_Admin::customer_login_enabled($customer); ?><span style="color:<?php echo $login_enabled?'#16804b':'#a36a00'; ?>;font-weight:700">● <?php echo $login_enabled?'Aktiv':'Inaktiv'; ?></span> · <?php echo esc_html($portal_email); ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline;margin-left:10px"><input type="hidden" name="action" value="9ls1_fotoportal_toggle_customer_login"><input type="hidden" name="aurora_workspace" value="1"><input type="hidden" name="client_id" value="<?php echo (int)$customer_id; ?>"><input type="hidden" name="enabled" value="<?php echo $login_enabled?'0':'1'; ?>"><?php wp_nonce_field('9ls1_fotoportal_toggle_customer_login'); ?><button type="submit" class="aurora-secondary-action"><?php echo $login_enabled?'Deaktiver innlogging':'Aktiver innlogging'; ?></button></form><?php elseif($portal_email): ?><span style="color:#9a6a00;font-weight:700">○ Ikke opprettet</span><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline;margin-left:8px"><input type="hidden" name="action" value="9ls1_fotoportal_ensure_customer_login"><input type="hidden" name="aurora_workspace" value="1"><input type="hidden" name="client_id" value="<?php echo (int)$customer_id; ?>"><?php wp_nonce_field('9ls1_fotoportal_ensure_customer_login'); ?><button type="submit" class="aurora-secondary-action">Opprett innlogging</button></form><?php else: ?><span style="color:#a33">Mangler e-post</span><?php endif; ?></dd></div>
...
<div><dt>Adresse</dt><dd><?php echo esc_html($customer->address ?: '—'); ?></dd></div><div><dt>Postnr. / sted</dt><dd><?php echo esc_html(trim(($customer->postal_code ?: '').' '.($customer->city ?: '')) ?: '—'); ?></dd></div><div><dt>Org.nr.</dt><dd><?php echo esc_html($customer->organization_number ?: '—'); ?></dd></div><div><dt>Fakturaadresse</dt><dd><?php echo !empty($customer->billing_same_as_customer)?'Samme som kundeadresse':esc_html(trim(($customer->billing_address ?: '').', '.($customer->billing_postal_code ?: '').' '.($customer->billing_city ?: ''),', ')); ?></dd></div></dl></section>                    </div>
                ...
                <?php $customer_login_url=NLS1_Fotoportal_Admin::customer_login_url(); ?><div style="border-top:1px solid #ece7f0;margin-top:22px;padding-top:20px"><div class="aurora-workspace-cardhead" style="margin-bottom:12px"><div><span class="aurora-workspace-eyebrow">FELLES KUNDEINNLOGGING</span><h3 style="margin:3px 0 4px">Fast innloggingsadresse</h3><p>Dette er adressen du kan gi kunden. Samme enkle innloggingsside brukes av alle Fotoportal-kunder, og Aurora sender kunden automatisk til riktig fotograf og kundeportal etter innlogging.</p></div><a class="aurora-secondary-action" href="<?php echo esc_url($customer_login_url); ?>" target="_blank">Åpne innlogging</a></div><div class="aurora-gallery-url-row"><input readonly value="<?php echo esc_attr($customer_login_url); ?>"><button type="button" class="aurora-secondary-action" data-copy-gallery-url="<?php echo esc_attr($customer_login_url); ?>">Kopier innloggingsadresse</button></div><p class="aurora-form-help">Tekniske forhåndsvisnings- og tokenlenker vises ikke til fotografen. Knappen «Åpne som kunde» ovenfor brukes bare av fotografen til forhåndsvisning.</p></div><?php if(!empty($customer->is_test) && class_exists('NLS1_Aurora_Demo_Content')):$ds=NLS1_Aurora_Demo_Content::account_state((int)$account->id);?><div class="aurora-demo-customer-login"><span class="aurora-workspace-eyebrow">DEMO-INNLOGGING</span><h3>Se kundeportalen som kunde</h3><div class="aurora-demo-credentials"><div><small>Brukernavn</small><strong><?php echo esc_html($ds['demo_customer_username']??''); ?></strong></div><div><small>Passord</small><strong><?php echo esc_html($ds['demo_customer_password']??''); ?></strong></div></div><p>DEMO-kundeportalen kan åpnes direkte uten å logge fotografen ut. Aurora bruker en sikker DEMO-kundekontekst.</p><?php $demo_preview_url=class_exists('NLS1_Aurora_Demo_Journey')?NLS1_Aurora_Demo_Journey::preview_url($customer_id):$cpurl; ?><a class="aurora-primary-action" href="<?php echo esc_url($demo_preview_url); ?>" target="_blank" rel="noopener">Se DEMO kundeportal ↗</a></div><?php endif;?>
                </section>
...
<th></th></tr></thead>
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="nls1-form aurora-step-form" id="aurora-new-project-form" enctype="multipart/form-data">
                        ...
                        <section class="aurora-form-step" data-step="2" hidden>
                            ...
                            <div class="nls1-form-grid">
                                ...
                                <label class="nls1-full">Notater<textarea name="description" rows="5" placeholder="Praktiske notater, ønsker, tidspunkt eller annen prosjektinformasjon."></textarea></label><div class="nls1-full aurora-optional-document"><span class="aurora-workspace-eyebrow">GALLERITILGANG</span><h3>Skal kunden måtte signere avtale før galleriet åpnes?</h3><p>Standard er Ja. Velg Nei for oppdrag der kontrakt håndteres på annen måte eller ikke er nødvendig.</p><label class="nls1-checkbox"><input type="checkbox" name="contract_required" value="1" checked> Ja – krev signert avtale før kundevisning av galleri</label></div>
            ...
            <?php elseif ($view === 'projects') : ?>
                ...
                <?php if ($project_id && $project) : ?>
                    ...
                    $contract_required = NLS1_Fotoportal_Admin::project_requires_contract($project_id);
                    $gallery_access_allowed = NLS1_Fotoportal_Admin::gallery_access_allowed($project_id);
                    $gallery_ready = false;
                    ...
                    elseif ($has_signed_contract && $gallery_ready) { $status_class='is-ready'; $status_help='Kontrakt er signert og minst ett galleri er klart. Kontroller betaling og levering i Digital levering.'; }
                    ...
                    <div class="aurora-customer-profile-head">
                        ...
                        <div class="aurora-project-profile-card">
                            ...
                            <div class="aurora-customer-profile-actions">
                                ...
                                <?php if($has_hq_delivery): ?><a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('hq_delivery',['project_id'=>$project_id]).'#aurora-demo-payment'); ?>"><span class="dashicons dashicons-money-alt"></span> Faktura / betaling</a><?php endif; ?>
                            </div>
                        </div>
                    </div>

                    <section class="aurora-workspace-card aurora-project-workflow aurora-project-workflow-top">
                        <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">PROSJEKTFLYT</span><h2>Fra oppdrag til ferdig leveranse</h2></div></div>
                        ...
                        <div class="aurora-project-steps aurora-project-steps-six">
                            ...
                            <a class="<?php echo $flow['contract_registered']?'is-complete':($contract_required?'is-attention':''); ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('contracts',['project_id'=>$project_id])); ?>"><span>2</span><strong>Kontrakt registrert</strong><small><?php echo $flow['contract_registered']?'Registrert ✓':($contract_required?'Neste steg':'Valgfritt'); ?></small></a>
                            <a class="<?php echo $flow['contract_signed']?'is-complete':($contract_required&&$flow['contract_registered']?'is-attention':''); ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('contracts',['project_id'=>$project_id])); ?>"><span>3</span><strong>Kontrakt signert</strong><small><?php echo $flow['contract_signed']?'Signert ✓':($contract_required?($flow['contract_registered']?'Venter på kunde':'Venter'):'Valgfritt'); ?></small></a>
                            <a class="<?php echo $flow['gallery']?'is-complete':($gallery_access_allowed?'is-attention':''); ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id])); ?>"><span>4</span><strong>Galleri</strong><small><?php echo $flow['gallery']?count($galleries).' gallerier ✓':($gallery_access_allowed?'Last opp bilder':'Låst'); ?></small></a>
                            <a class="<?php echo $flow['documents']?'is-complete':''; ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('documents',['project_id'=>$project_id])); ?>"><span>5</span><strong>Dokumenter</strong><small><?php echo $flow['documents']?count($documents).' filer ✓':'Valgfritt'; ?></small></a>
                            <a class="<?php echo $flow['paid']?'is-complete':''; ?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url($has_hq_delivery?'hq_delivery':'projects',['project_id'=>$project_id])); ?>"><span>6</span><strong>Leveranse</strong><small><?php echo $flow['paid']?'Klar ✓':'Venter'; ?></small></a>
                        </div>
                        <div class="aurora-status-legend"><span><i class="is-green"></i>Fullført</span><span><i class="is-blue"></i>Aktivt steg</span><span><i class="is-gray"></i>Venter / valgfritt</span></div>
                    </section>

                    <section class="aurora-workspace-card aurora-contract-gate-card"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">AVTALEKRAV</span><h2>Tilgang til kundegalleri</h2><p>Bestem om kunden må ha signert avtale før galleriet kan åpnes. Betaling og HQ-levering styres separat.</p></div><span class="aurora-status-pill <?php echo $gallery_access_allowed?'is-active':''; ?>"><?php echo $contract_required?'Avtale kreves':'Avtale valgfri'; ?></span></div><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-inline-form aurora-contract-gate-form" id="aurora-contract-gate-form"><?php wp_nonce_field('9ls1_fotoportal_save_project_contract_gate'); ?><input type="hidden" name="action" value="9ls1_fotoportal_save_project_contract_gate"><input type="hidden" name="aurora_workspace" value="1"><input type="hidden" name="project_id" value="<?php echo (int)$project_id; ?>"><label class="nls1-checkbox"><input type="checkbox" name="contract_required" value="1" <?php checked($contract_required); ?> onchange="this.form.querySelector('.aurora-gate-save-state').textContent='Lagrer …';this.form.submit();"> Krev signert avtale før kundevisning av galleri</label><span class="aurora-gate-save-state">Endringen lagres automatisk</span></form><?php if(!$contract_required): ?><p class="aurora-workspace-help"><span class="dashicons dashicons-info-outline"></span>Galleri kan åpnes uten signert avtale. Endringen er logget på prosjektet.</p><?php endif; ?></section>

                    <?php
                    $next_title='Opprett og send kontrakt'; $next_text='Registrer avtalen for prosjektet. Når kontrakten er sendt, kan kunden signere den i kundeportalen.'; $next_label='Gå til kontrakt'; $next_url=NLS1_Photographer_Workspace::url('contracts',['project_id'=>$project_id]); $next_icon='dashicons-media-document';
                    if($gallery_ready && !$gallery_access_allowed){$next_title='Galleri klart · låst for kunde';$next_text='Bildene er behandlet, men dette prosjektet krever signert avtale før kundevisning.';$next_label='Gå til kontrakt';$next_icon='dashicons-lock';}
                    elseif($contract_required && $flow['contract_registered'] && !$has_signed_contract){$next_title='Venter på kundens signatur';$next_text='Prosjektet krever signert avtale før kundevisning. Du kan endre dette kravet på prosjektet.';$next_label='Se kontrakt';$next_icon='dashicons-email-alt';}
                    elseif($gallery_access_allowed && !$flow['gallery']){$next_title=$contract_required?'Avtalen er godkjent – last opp bilder':'Galleri kan opprettes uten signert avtale';$next_text='Last opp høyoppløselige originalbilder. Aurora lager automatisk preview og thumbnails; HQ-originalene leveres uten vannmerke når leveransen frigis.';$next_label='Gå til prosjektgallerier';$next_url=NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id,'new_gallery'=>1]);$next_icon='dashicons-format-gallery';}
                    elseif($flow['gallery'] && !$flow['documents']){$next_title='Galleri klart · tilgjengelig for kunde';$next_text='Kontrakten er signert og galleriet kan brukes i kundereisen. Tilleggsdokumenter er valgfritt før leveranse.';$next_label='Gå til prosjektgallerier';$next_url=NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id]);$next_icon='dashicons-unlock';}
                    elseif($flow['gallery'] && $flow['documents']){$next_title='Klargjør leveransen';$next_text='Prosjekt, kontrakt, galleri og dokumenter er på plass. Kontroller leveransen før den frigis til kunden.';$next_label='Gå til leveranse';$next_url=NLS1_Photographer_Workspace::url($has_hq_delivery?'hq_delivery':'projects',['project_id'=>$project_id]);$next_icon='dashicons-download';}
                    $preview=[]; foreach($galleries as $pg){foreach(NLS1_Fotoportal_Admin::get_gallery_images((int)$pg->id,4) as $pi){if(!empty($pi->thumbnail_url)||!empty($pi->preview_url))$preview[]=$pi;if(count($preview)>=4)break;}if(count($preview)>=4)break;} $direct_gallery=!empty($galleries)?reset($galleries):null; $customer_portal_url=NLS1_Fotoportal_Admin::photographer_customer_preview_url((int)$project->client_id); if(!empty($project->is_test)&&class_exists('NLS1_Aurora_Demo_Journey')&&NLS1_Aurora_Demo_Journey::is_demo_project($project_id))$customer_portal_url=NLS1_Aurora_Demo_Journey::preview_url((int)$project->client_id);
                    ?>
                    <section class="aurora-workspace-card aurora-project-next-step">
                      <div class="aurora-next-step-icon"><span class="dashicons <?php echo esc_attr($next_icon); ?>"></span></div><div class="aurora-next-step-copy"><span class="aurora-workspace-eyebrow">NESTE STEG</span><h2><?php echo esc_html($next_title); ?></h2><p><?php echo esc_html($next_text); ?></p></div><a class="aurora-primary-action" href="<?php echo esc_url($next_url); ?>"><?php echo esc_html($next_label); ?> →</a>
                    </section>
                    <?php if($preview): ?><section class="aurora-workspace-card aurora-project-gallery-preview"><div><span class="aurora-workspace-eyebrow">PROSJEKTGALLERI</span><h2>Bildene i prosjektet</h2><p><?php echo count($galleries); ?> galleri · <?php echo array_sum(array_map(fn($g)=>(int)$g->original_count,$galleries)); ?> bilder · <?php echo $gallery_access_allowed?'🔓 tilgjengelig for kunde':'🔒 låst for kunde'; ?></p><div class="aurora-project-gallery-actions"><a class="aurora-secondary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id])); ?>">Gå til prosjektgallerier →</a><?php if($direct_gallery): ?><a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id,'gallery_id'=>(int)$direct_gallery->id])); ?>">Åpne <?php echo esc_html($direct_gallery->gallery_title); ?> →</a><?php endif; ?><?php if($customer_portal_url): ?><a class="aurora-secondary-action" href="<?php echo esc_url($customer_portal_url); ?>" target="_blank" rel="noopener">Åpne kundeinnlogging ↗</a><?php endif; ?></div></div><div class="aurora-project-gallery-thumbs"><?php foreach($preview as $pi): ?><img src="<?php echo esc_url($pi->thumbnail_url?:$pi->preview_url); ?>" alt=""><?php endforeach; ?></div></section><?php endif; ?>
                    ...
                    <section class="aurora-workspace-card aurora-project-danger-zone"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">SIKKER SLETTING</span><h2>Slett prosjekt permanent</h2><p>Sletter prosjektet, kontrakter, dokumenter, gallerier, bilder, kommentarer, favoritter, logger og genererte leveransefiler. Kunden beholdes.</p></div></div><details><summary>Slett dette prosjektet</summary><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('Dette kan ikke angres. Prosjektet og alle tilhørende bilder/filer slettes permanent. Fortsette?');"><input type="hidden" name="action" value="9ls1_fotoportal_delete_project_permanently"><input type="hidden" name="project_id" value="<?php echo (int)$project_id; ?>"><?php wp_nonce_field('9ls1_fotoportal_delete_project_permanently'); ?><label>Skriv prosjektnavnet for å bekrefte<input type="text" name="confirm_project_name" placeholder="<?php echo esc_attr($project->project_name); ?>" required></label><button class="aurora-danger-action" type="submit">Slett prosjekt og alle prosjektdata permanent</button></form></details></section>
...
<th></th></tr></thead>
            ...
            <?php elseif ($view === 'contracts') : ?>
                <?php if (!empty($_GET['message']) && $_GET['message']==='contract_sent') : ?><div class="aurora-workspace-alert is-success"><span class="dashicons dashicons-yes-alt"></span><div><strong>Avtalen er sendt til e-postsystemet</strong><p>Utsendingen er registrert i E-posthistorikk nedenfor. Der ser du mottaker, tidspunkt og resultat.</p></div></div><?php endif; ?>
                <?php if (!empty($_GET['message']) && $_GET['message']==='contract_mail_failed') : ?><div class="aurora-workspace-alert"><span class="dashicons dashicons-warning"></span><div><strong>Kontrakten ble ikke sendt</strong><p>E-postutsendingen feilet. Kontroller mottakeradresse og e-postoppsett før du prøver igjen. Status er ikke satt til Sendt.</p></div></div><?php endif; ?>
                <?php
                $project_id = absint($_GET['project_id'] ?? 0);
                $project = $project_id ? NLS1_Fotoportal_Admin::get_project($project_id) : null;
                $contracts = $project ? NLS1_Fotoportal_Admin::get_project_contracts($project_id) : [];
                $contract_client = $project ? NLS1_Fotoportal_Admin::get_client((int)$project->client_id) : null;
                $contract_contact = $project ? NLS1_Fotoportal_Admin::get_primary_contact((int)$project->client_id) : null;
                ?>
                ...
                <?php if ($project) : ?>
                    <div class="aurora-contract-context">
                        ...
                        <div class="aurora-project-profile-card">
                            ...
                            <?php $render_project_gallery_status($project); ?>
                        </div>
                    </div>
                    ...
                    <div class="aurora-contract-grid">
                                    ...
                                    <?php foreach ($contracts as $contract) : ?>
                                        <div class="aurora-contract-item">
                                            ...
                                            <div class="aurora-row-actions">
                                                ...
                                                <?php endif; ?>
                                                <?php if (in_array($cstatus, ['draft','sent'], true) && (($contract->contract_source ?? 'aurora') !== 'upload')) : ?>
                                                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                                        ...
                                                        <button class="aurora-secondary-action" type="submit"><?php echo $cstatus === 'sent' ? 'Send på nytt' : 'Send til signering'; ?></button>
                                                    </form>
                                                <?php endif; ?>
                                                <?php if ($cstatus !== 'signed') : ?>
                                                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('Slette denne kontrakten permanent? Dette kan ikke angres.');">
                                                        <input type="hidden" name="action" value="9ls1_fotoportal_delete_contract"><input type="hidden" name="contract_id" value="<?php echo (int)$contract->id; ?>"><input type="hidden" name="aurora_workspace" value="1"><?php wp_nonce_field('9ls1_fotoportal_delete_contract_'.(int)$contract->id); ?>
                                                        <button class="aurora-secondary-action" type="submit" title="Slett kontrakt"><span class="dashicons dashicons-trash"></span>Slett</button>
                                                    </form>
                                                <?php else : ?><small title="Signerte avtaler beholdes som dokumentasjon">Signert avtale kan ikke slettes</small><?php endif; ?>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        <?php
                        $contract_mail_history = array_values(array_filter(NLS1_Fotoportal_Admin::get_logs(0, $project_id), function($row){
                            return in_array($row->log_type ?? '', ['contract_email','contract_email_error'], true);
                        }));
                        ?>
                        <section class="aurora-workspace-card aurora-contract-mail-history">
                            <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">E-POSTHISTORIKK</span><h2>Utsending av avtaler</h2><p>Viser hva WordPress rapporterte ved hver ADS-utsending. «Godkjent» betyr at e-postsystemet tok imot meldingen; faktisk levering styres av nettstedets e-postleverandør.</p></div></div>
                            <?php if ($contract_mail_history) : ?>
                                <div class="aurora-mail-history-list">
                                    <?php foreach (array_slice($contract_mail_history, 0, 20) as $mailrow) : ?>
                                        <div class="aurora-mail-history-row <?php echo ($mailrow->log_type === 'contract_email_error') ? 'is-error' : 'is-ok'; ?>">
                                            <span class="dashicons <?php echo ($mailrow->log_type === 'contract_email_error') ? 'dashicons-warning' : 'dashicons-email-alt'; ?>"></span>
                                            <div><strong><?php echo ($mailrow->log_type === 'contract_email_error') ? 'Utsending feilet' : 'E-post godkjent for utsending'; ?></strong><p><?php echo esc_html($mailrow->message); ?></p></div>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <div class="aurora-empty-state"><span class="dashicons dashicons-email-alt"></span><strong>Ingen utsendinger registrert ennå</strong><p>Når en avtale sendes eller sendes på nytt, vises resultatet her.</p></div>
                            <?php endif; ?>
                        </section>
                        ...
                        <section class="aurora-workspace-card">
                            ...
                            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-contract-form" enctype="multipart/form-data">
                                ...
                                <label>Signerer<input type="text" name="signer_name" value="<?php echo esc_attr($contract_contact ? trim(($contract_contact->first_name??'').' '.($contract_contact->last_name??'')) : ($contract_client->client_name??'')); ?>" placeholder="Navn"></label>
                                <label>E-post til signerer<input type="email" name="signer_email" value="<?php echo esc_attr($contract_contact->email ?? $contract_client->email ?? ''); ?>" placeholder="kunde@epost.no" required></label>
                    ...
                    <section class="aurora-workspace-card aurora-project-workflow">
                        <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">PROSJEKTFLYT</span><p>Dette er prosjektets faste steg og er separat fra Demo Journey.</p></div></div>
                        <div class="aurora-project-steps">
                            ...
                            <a href="<?php echo esc_url(NLS1_Photographer_Workspace::url($has_hq_delivery?'hq_delivery':'projects',['project_id'=>$project_id])); ?>"><span>5</span><strong>Leveranse</strong><small>Sluttlevering</small></a>
                        </div>
                    </section>
                ...
                <?php if ($project) : ?>
                    ...
                    <div class="aurora-project-profile-card">
                        ...
                        <div class="aurora-customer-profile-main">
                            <span class="aurora-workspace-eyebrow"><?php echo esc_html($project->project_number); ?></span>
                            <h2><?php echo esc_html($project->project_name); ?></h2>
                            <div class="aurora-customer-meta"><span><?php echo esc_html($project->client_name ?: '—'); ?></span></div>
                        </div>
                        <?php $render_project_gallery_status($project); ?>
                    </div>
            ...
            <?php elseif ($view === 'galleries') : ?>
                ...
                $gallery_unlocked = $project ? NLS1_Fotoportal_Admin::gallery_access_allowed($project_id) : false;
                $gallery_contract_required = $project ? NLS1_Fotoportal_Admin::project_requires_contract($project_id) : true;
                ?>
                ...
                <?php elseif ($gallery_message === 'gallery_contract_required') : ?>
                    <div class="aurora-workspace-alert"><span class="dashicons dashicons-lock"></span><div><strong>Signert avtale kreves for dette prosjektet</strong><p>Endre avtalekravet på prosjektet dersom kunden skal få galleri uten signert avtale.</p></div></div>
                <?php endif; ?>
                ...
                ?>
                    <a class="aurora-back-link" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('galleries',['project_id'=>$project_id])); ?>"><span class="dashicons dashicons-arrow-left-alt2"></span>Tilbake til gallerier</a><div class="aurora-context-links"><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('projects',['project_id'=>$project_id])); ?>">Åpne prosjekt</a><?php if($has_favorites_comments): ?><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('selections',['gallery_id'=>(int)$detail_gallery->id])); ?>">Åpne Bildevalg</a><?php endif; ?><?php if($has_hq_delivery): ?><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('hq_delivery',['project_id'=>$project_id])); ?>">Åpne Digital levering</a><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('hq_delivery',['project_id'=>$project_id]).'#aurora-demo-payment'); ?>">Faktura / betaling</a><?php endif; ?></div>
                    <section class="aurora-workspace-card aurora-gallery-detail-head">
                        ...
                        <div class="aurora-gallery-detail-actions">
                            ...
                            <a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Fotoportal_Admin::photographer_customer_preview_url((int)$detail_gallery->client_id,['gallery_id'=>(int)$detail_gallery->id])); ?>" target="_blank" rel="noopener"><span class="dashicons dashicons-external"></span>Se kundegalleri</a><?php if($has_customer_portal): ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-inline-form"><input type="hidden" name="action" value="9ls1_fotoportal_send_customer_portal"><input type="hidden" name="gallery_id" value="<?php echo (int)$detail_gallery->id; ?>"><input type="hidden" name="aurora_workspace" value="1"><?php wp_nonce_field('9ls1_fotoportal_send_customer_portal'); ?><button class="aurora-secondary-action" type="submit"><span class="dashicons dashicons-email-alt"></span>Send URL til kunde</button></form><?php endif; ?>
                        </div>
                    </section>

                    <section class="aurora-workspace-card aurora-gallery-edit-card"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">GALLERIINFO</span><h2>Navn og beskrivelse</h2><p>Dette er teksten kunden ser i galleriet. Kundens navn brukes ikke som beskrivelse.</p></div></div><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-gallery-edit-form"><input type="hidden" name="action" value="9ls1_fotoportal_update_gallery_details"><input type="hidden" name="gallery_id" value="<?php echo (int)$detail_gallery->id; ?>"><input type="hidden" name="aurora_workspace" value="1"><?php wp_nonce_field('9ls1_fotoportal_update_gallery_details'); ?><label>Gallerinavn<input type="text" name="gallery_title" value="<?php echo esc_attr($detail_gallery->gallery_title); ?>" required></label><label>Beskrivelse<textarea name="gallery_description" rows="3" placeholder="Skriv en kort beskrivelse av galleriet …"><?php echo esc_textarea($detail_gallery->gallery_description??''); ?></textarea></label><div class="aurora-form-grid"><label>Nedlastbar til<input type="date" name="downloadable_until" value="<?php echo esc_attr($detail_gallery->downloadable_until??''); ?>"></label><label>Automatisk slettedato<input type="date" name="auto_delete_at" value="<?php echo esc_attr($detail_gallery->auto_delete_at??''); ?>"></label></div><p class="aurora-form-help">Datoene kan endres når som helst. Automatisk sletting gjelder galleriets lagrede filer og skal derfor settes med omtanke.</p><button class="aurora-primary-action" type="submit">Lagre galleriinfo</button></form></section>
                    ...
                    <section class="aurora-workspace-card aurora-gallery-share-card">
                        ...
                        <?php $customer_portal_url=$has_customer_portal ? NLS1_Fotoportal_Admin::customer_login_url() : ''; ?>
                        ...
                        </div><?php if($has_customer_portal): ?><div class="aurora-gallery-url-row aurora-customer-portal-row"><input type="text" readonly value="<?php echo esc_attr($customer_portal_url); ?>"><button type="button" class="aurora-secondary-action" data-copy-gallery-url="<?php echo esc_attr($customer_portal_url); ?>">Kopier kundeinnlogging</button></div><?php endif; ?>
                    </section>
                    ...
                    <?php if($selection_history): ?><details class="aurora-workspace-card aurora-selection-history"><summary><strong>Historikk – tilbaketrukne redigeringsønsker</strong> <small><?php echo count($selection_history); ?> registrert</small></summary><div class="aurora-resource-list"><?php foreach($selection_history as $hist): ?><div><span class="dashicons dashicons-backup"></span><div><strong><?php echo esc_html($hist->project_name.' · '.$hist->gallery_title); ?></strong><small><?php echo esc_html($hist->comment_text); ?> · trukket tilbake <?php echo esc_html(wp_date('d.m.Y H:i',strtotime($hist->deleted_at))); ?></small></div></div><?php endforeach; ?></div></details><?php endif; ?>
                ...
                <section class="aurora-workspace-card aurora-gallery-toolbar">
                    ...
                    <?php if (!$project) : ?><p>Velg et prosjekt for å opprette et nytt galleri. Oversikten nedenfor viser gallerier på denne fotografkontoen.</p><div class="aurora-hq-upload-note"><strong>Last opp HIGH QUALITY-originaler.</strong><span>Aurora lagrer originalene, lager automatisk preview og thumbnails, og bruker vannmerke kun på kundevisningen. Når levering frigis, får kunden HQ-bildene uten vannmerke.</span></div><?php endif; ?><?php if(!$project): ?><div class="aurora-gallery-watermark-strip"><div><span class="aurora-workspace-eyebrow">VANNMERKE-PREVIEW</span><strong>Slik vises vannmerket på preview-bildene</strong><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('settings').'#vannmerke'); ?>">Rediger vannmerke →</a></div><div class="aurora-dashboard-watermark-preview is-compact" data-position="<?php echo esc_attr($photographer_profile['watermark_position']??'bottom_right'); ?>" style="--wm-size:<?php echo (int)($photographer_profile['watermark_size']??18); ?>%;--wm-opacity:<?php echo (100-(int)($photographer_profile['watermark_opacity']??35))/100; ?>"><img src="<?php echo esc_url($branding['watermark_preview_url']); ?>" alt="Aurora vannmerke-preview"><?php if(!empty($photographer_profile['watermark_url'])):?><img class="wm" src="<?php echo esc_url($photographer_profile['watermark_url']); ?>" alt="Vannmerke"><?php else:?><span class="wm text">DITT VANNMERKE</span><?php endif;?></div></div><?php endif;?>
                </section>
                ...
                <?php if ($project) : ?>
                        ...
                        <section id="aurora-new-gallery" class="aurora-workspace-card aurora-gallery-upload-card">
                            ...
                            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" class="aurora-gallery-upload-form">
                                ...
                                <div class="aurora-form-grid">
                                    ...
                                    <label>HIGH QUALITY-bilder<input type="file" name="gallery_images[]" accept=".jpg,.jpeg,.png,.webp,.tif,.tiff,image/jpeg,image/png,image/webp,image/tiff" multiple></label><label>ZIP-fil <small>Valgfritt</small><input type="file" name="gallery_zip" accept=".zip"></label>
                                ...
                                <p class="aurora-form-help"><strong>Last opp ferdige HIGH QUALITY-bilder direkte</strong> – velg gjerne mange samtidig. Har du allerede samlet bildene i en ZIP, kan den brukes i stedet. Aurora lagrer HQ-originalene og lager automatisk preview, thumbnails og leveransefiler. Vannmerke brukes bare på kundevisningen; kunden får HQ-bilder uten vannmerke ved nedlasting.</p>
                    ...
                    <?php elseif (!$gallery_unlocked) : ?>
                        <?php $is_seeded_demo_gallery = !empty($project->is_test) && empty($contracts = NLS1_Fotoportal_Admin::get_project_contracts($project_id)) && (!class_exists('NLS1_Aurora_Demo_Journey') || !NLS1_Aurora_Demo_Journey::is_demo_project($project_id)); ?>
                        <?php if($is_seeded_demo_gallery): ?>
                            <div class="aurora-gallery-lock-hint"><span class="dashicons dashicons-info-outline"></span><span><strong>Dette er eksempelgalleriet fra Demo Content Pack.</strong> Det har ingen signert avtale og teller ikke som et steg i Demo Journey. <a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('dashboard',['demo_journey'=>1])); ?>">Gå til Demo Guide →</a></span></div>
                        <?php else: ?>
                            <div class="aurora-gallery-lock-hint"><span class="dashicons dashicons-lock"></span><span><strong>Galleriet venter på signert kontrakt.</strong> <a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('contracts',['project_id'=>$project_id])); ?>">Gå til kontrakt →</a></span></div>
                        <?php endif; ?>
                    <?php endif; ?>
            ...
            <?php elseif ($view === 'selections') : ?>
                ...
                $selection_history = NLS1_Fotoportal_Admin::photographer_selection_history((int)$account->id);
                ...
                foreach($selection_items as $si){
                    ...
                    if((int)($si->edit_request_count??0)>0)$selection_counts['comments']++;
                ...
                <?php if($selection_items): ?><div class="aurora-selection-workbench"><div class="aurora-selection-grid" data-selection-grid><?php foreach($selection_items as $si): $img=$si->thumbnail_url?:$si->preview_url; if(!$img)continue; ?>
                    <article class="aurora-selection-card" data-favorite="<?php echo !empty($si->is_favorite)?'1':'0'; ?>" data-approved="<?php echo !empty($si->is_selected)?'1':'0'; ?>" data-comment="<?php echo (int)($si->edit_request_count??0)>0?'1':'0'; ?>" data-client="<?php echo (int)$si->client_id; ?>" data-project="<?php echo (int)$si->project_id; ?>" data-gallery="<?php echo (int)$si->gallery_id; ?>" data-status="<?php echo esc_attr($si->selection_status?:'open'); ?>">
                        <button type="button" class="aurora-selection-image" data-gallery-image="<?php echo esc_url($si->preview_url?:$img); ?>"><img loading="lazy" src="<?php echo esc_url($img); ?>" alt=""><span><?php if(!empty($si->is_favorite)): ?><i>♡</i><?php endif; ?><?php if(!empty($si->is_selected)): ?><i>✓</i><?php endif; ?><?php if((int)($si->edit_request_count??0)): ?><i>💬 <?php echo (int)$si->edit_request_count; ?></i><?php endif; ?></span></button>
                        <div class="aurora-selection-body"><?php if(!empty($photographer_profile['show_filenames'])):?><strong><?php echo esc_html($si->original_filename); ?></strong><?php endif;?><small><?php echo esc_html($si->client_name?:'Ukjent kunde'); ?> · <?php echo esc_html($si->project_name); ?></small><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('galleries',['project_id'=>(int)$si->project_id,'gallery_id'=>(int)$si->gallery_id])); ?>"><?php echo esc_html($si->gallery_title); ?> <span class="dashicons dashicons-arrow-right-alt2"></span></a><a href="<?php echo esc_url(NLS1_Photographer_Workspace::url('projects',['project_id'=>(int)$si->project_id])); ?>">Åpne prosjekt <span class="dashicons dashicons-arrow-right-alt2"></span></a><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-selection-status-form"><input type="hidden" name="action" value="9ls1_fotoportal_update_selection_status"><input type="hidden" name="gallery_id" value="<?php echo (int)$si->gallery_id; ?>"><?php wp_nonce_field('9ls1_fotoportal_update_selection_status'); ?><select name="selection_status" onchange="this.form.submit()"><option value="open" <?php selected($si->selection_status?:'open','open'); ?>>Ingen forespørsel</option><option value="submitted" <?php selected($si->selection_status,'submitted'); ?>>Redigeringsønske</option><option value="processing" <?php selected($si->selection_status,'processing'); ?>>Under behandling</option><option value="ready" <?php selected($si->selection_status,'ready'); ?>>Ferdig behandlet</option></select></form><?php $thread=NLS1_Fotoportal_Admin::image_comment_thread((int)$si->image_id); if($thread): ?><div class="aurora-selection-comment" style="display:block"><strong style="display:block;margin-bottom:7px">Kommentartråd · <?php echo count($thread); ?></strong><?php foreach($thread as $tc): ?><div style="border-top:1px solid #ece7ef;padding:7px 0"><small><?php echo esc_html($tc->author_type==='photographer'?'Fotograf':'Kunde'); ?><?php echo !empty($tc->is_edit_request)?' · Redigeringsønske':''; ?> · <?php echo esc_html(mysql2date('d.m.Y H:i',$tc->created_at)); ?></small><p style="margin:3px 0 0"><?php echo esc_html($tc->comment_text); ?></p></div><?php endforeach; ?></div><?php endif; ?><?php if((int)($si->edit_request_count??0)>0): ?><div style="margin-top:10px;border:1px solid #e8e2eb;border-radius:10px;padding:10px;background:#faf9fb"><?php if(($si->edit_status??'none')==='ready'&&!empty($si->edited_url)): ?><label style="display:flex;gap:8px;align-items:center;color:#157849;font-weight:700"><input type="checkbox" checked disabled> Redigeringsønske utført · redigert bilde lastet opp</label><?php else: ?><strong style="display:block;margin-bottom:7px">Fullfør redigeringsønske</strong><form method="post" enctype="multipart/form-data" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="9ls1_fotoportal_upload_edited_image"><input type="hidden" name="image_id" value="<?php echo (int)$si->image_id; ?>"><?php wp_nonce_field('9ls1_fotoportal_upload_edited_image'); ?><input type="file" name="edited_image" accept="image/jpeg,image/png,image/webp,image/tiff" required style="max-width:100%;margin-bottom:7px"><button class="aurora-secondary-action" type="submit" style="width:100%">Last opp ferdig redigert bilde</button></form><?php endif; ?></div><?php endif; ?></div>
                    </article>
                <?php endforeach; ?></div><aside class="aurora-selection-sticky"><span class="aurora-workspace-eyebrow">VIDERE BEHANDLING</span><h3>Aktivt bildevalg</h3><div class="aurora-selection-stats"><span>♡ Favoritter <b><?php echo (int)$selection_counts['favorites'];?></b></span><span>✓ Valgte <b><?php echo (int)$selection_counts['approved'];?></b></span><span>💬 Redigeringsønsker <b><?php echo (int)$selection_counts['comments'];?></b></span></div><p>Rask tilgang til prosjekt og levering.</p><?php if(count($selection_projects)===1): $only_pid=(int)array_key_first($selection_projects); ?><a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('projects',['project_id'=>$only_pid]));?>">Åpne prosjekt</a><a class="aurora-secondary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('hq_delivery',['project_id'=>$only_pid]));?>">Digital levering</a><?php endif;?></aside></div><div class="aurora-selection-empty-filter" data-selection-empty hidden>Ingen bilder matcher filteret.</div><?php else: ?><div class="aurora-empty-state"><span class="dashicons dashicons-yes-alt"></span><strong>Ingen kundeaktivitet ennå</strong><p>Favoritter, valgte bilder og kommentarer vil automatisk vises her.</p></div><?php endif; ?>
            ...
            <?php elseif ($view === 'hq_delivery') : ?>
                ...
                $delivery_contract_required = $project ? NLS1_Fotoportal_Admin::project_requires_contract($project_id) : true;
                $delivery_flow = $project ? NLS1_Fotoportal_Admin::project_delivery_state($project_id) : [];
                $paid = !empty($delivery_flow['paid']);
                $edit_state = $project ? NLS1_Fotoportal_Admin::project_edit_request_state($project_id) : ['total'=>0,'done'=>0,'open'=>0,'items'=>[]];
                $delivery_images = $project ? NLS1_Fotoportal_Admin::project_delivery_images($project_id) : [];
                $delivery_released = $project && !empty($project->delivery_released_at) && (($project->delivery_status??'')==='released');
                $is_demo_delivery = $project && class_exists('NLS1_Aurora_Demo_Journey') && NLS1_Aurora_Demo_Journey::is_demo_project($project_id);
                $demo_delivery_state = $is_demo_delivery ? NLS1_Aurora_Demo_Journey::state() : [];
                $ready_galleries = 0; $download_galleries = 0; $image_total = 0;
                $download_stats = $project ? NLS1_Fotoportal_Admin::project_download_stats($project_id) : ['count'=>0,'last_at'=>''];
                $download_state_label = !empty($download_stats['count']) ? 'Lastet ned' : ($delivery_released ? (!empty($project->delivery_terms_accepted_at) ? 'Klar' : 'Venter vilkår') : 'Ikke frigitt');
                ...
                ?>
                <?php if ($project) : ?>
                    <?php if($is_demo_delivery): ?><section class="aurora-workspace-card aurora-demo-delivery-helper"><span class="aurora-demo-badge">GUIDET DEMO</span><h2>Test leveransen som på et ekte prosjekt</h2><p><strong>Sirklene under er status – de kan ikke klikkes.</strong> Gjør handlingene lenger ned på denne siden, så blir de grønne automatisk.</p><div class="aurora-demo-illustration"><span><?php echo !empty($demo_delivery_state['delivery_rights_tested'])?'✓':'○'; ?> Bruksrett lagret</span><span><?php echo $paid?'✓':'○'; ?> Faktura betalt</span></div><div class="aurora-step-actions"><a class="aurora-primary-action" href="#aurora-demo-rights">1. Gå til Bruksrett ↓</a><a class="aurora-secondary-action" href="#aurora-demo-payment">2. Gå til Faktura ↓</a></div><p><small>Leveringsbetingelser godkjennes av kunden før HQ-nedlasting og skal derfor ikke være grønne ennå.</small></p><?php if(!empty($demo_delivery_state['delivery_rights_tested'])&&$paid): ?><a class="aurora-primary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('dashboard',['demo_journey'=>1,'demo_step'=>'complete'])); ?>">Fortsett Demo Journey →</a><?php else: ?><a class="aurora-secondary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('dashboard',['demo_journey'=>1,'demo_step'=>'delivery'])); ?>">Tilbake til Demo Guide</a><?php endif; ?></section><?php endif; ?>
                    <a class="aurora-back-link" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('projects',['project_id'=>$project_id])); ?>"><span class="dashicons dashicons-arrow-left-alt2"></span>Tilbake til prosjekt</a>
                    <div class="aurora-project-profile-card">
                    ...
                    <div class="aurora-delivery-summary-grid">
                        ...
                        <section class="aurora-workspace-card aurora-delivery-summary"><span class="dashicons dashicons-download"></span><div><small>Nedlasting</small><strong><?php echo esc_html($download_state_label); ?></strong><p><?php echo !empty($download_stats['count']) ? ((int)$download_stats['count'].' nedlasting'.((int)$download_stats['count']===1?'':'er').' · sist '.esc_html(wp_date('d.m.Y H:i',strtotime($download_stats['last_at'])))) : 'HQ-leveransen spores når kunden laster ned.'; ?></p></div></section>
                        <section class="aurora-workspace-card aurora-delivery-summary"><span class="dashicons dashicons-media-document"></span><div><small>Kontrakt</small><strong><?php echo $delivery_contract_required ? ($signed ? 'Signert' : 'Mangler') : 'Valgfri'; ?></strong><p><?php echo $delivery_contract_required ? ($signed ? 'Avtalekravet er oppfylt.' : 'Signering må fullføres før kundevisning.') : 'Prosjektet krever ikke signert avtale for galleri.'; ?></p></div></section>
                    </div>

                    <section class="aurora-workspace-card">
                        <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">STEG 5</span><h2>Leveranse</h2><p>Klargjør prosjektet for kundeportal, godkjenning og endelig levering.</p><?php if(!empty($download_stats['count'])): ?><p><strong>✓ Kunden har lastet ned bildene.</strong> Sist registrert <?php echo esc_html(wp_date('d.m.Y H:i',strtotime($download_stats['last_at']))); ?>.</p><?php endif; ?></div><span class="aurora-status-pill <?php echo $project->status === 'delivered' ? 'is-active' : ''; ?>"><?php echo esc_html(NLS1_Fotoportal_Admin::status_label($project->status)); ?></span></div>
                        <?php if ($project_galleries) : ?>
                            ...
                            ?>
                                <div class="aurora-delivery-gallery-row">
                                    ...
                                    <div><span>Nedlasting</span><strong><?php echo esc_html($download_state_label); ?></strong></div>
                    ...
                    <?php if($has_favorites_comments && !empty($edit_state['total'])): ?>
                    <section class="aurora-workspace-card">
                        <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">BILDEVALG</span><h2>Redigerte bilder etter ønske</h2><p>Kundens redigeringsønsker må være ferdigbehandlet før Digital levering kan frigis.</p></div><span class="aurora-status-pill <?php echo empty($edit_state['open'])?'is-active':''; ?>"><?php echo (int)$edit_state['done']; ?> / <?php echo (int)$edit_state['total']; ?> ferdige</span></div>
                        <div class="aurora-delivery-gallery-list">
                        <?php foreach($edit_state['items'] as $edit): $done=(($edit->edit_status??'none')==='ready'&&!empty($edit->edited_url)); ?>
                            <div class="aurora-delivery-gallery-row" style="grid-template-columns:64px minmax(180px,1fr) minmax(180px,1.3fr) minmax(240px,1.4fr)">
                                <div><?php if($edit->thumbnail_url||$edit->preview_url): ?><img src="<?php echo esc_url($edit->thumbnail_url?:$edit->preview_url); ?>" alt="" style="width:56px;height:56px;object-fit:cover;border-radius:8px"><?php endif; ?></div>
                                <div><strong><?php echo esc_html($edit->original_filename); ?></strong><small><?php echo $done?'Ferdig redigert':'Redigering ønsket'; ?></small></div>
                                <div><span>Dialog om bildet</span><?php $thread=NLS1_Fotoportal_Admin::image_comment_thread((int)$edit->id); ?><div class="aurora-comment-thread"><?php foreach($thread as $comment): ?><div class="aurora-comment-row"><small><strong><?php echo (($comment->author_type??'customer')==='photographer')?'Fotograf':'Kunde'; ?></strong> · <?php echo esc_html($comment->created_at); ?></small><p><?php echo esc_html($comment->comment_text); ?></p><?php if(($comment->author_type??'customer')==='photographer'): ?><form class="aurora-comment-actions" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="9ls1_fotoportal_delete_image_comment"><input type="hidden" name="comment_id" value="<?php echo (int)$comment->id; ?>"><?php wp_nonce_field('9ls1_fotoportal_delete_image_comment'); ?><button class="button-link-delete" type="submit">Slett min kommentar</button></form><?php endif; ?></div><?php endforeach; ?></div><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="9ls1_fotoportal_add_image_comment"><input type="hidden" name="image_id" value="<?php echo (int)$edit->id; ?>"><?php wp_nonce_field('9ls1_fotoportal_add_image_comment'); ?><textarea name="comment_text" rows="2" placeholder="Svar kunden …" required></textarea><button class="button" type="submit">Legg til svar</button></form></div>
                                <div><?php if($done): ?><span style="color:#159455;font-weight:700">✓ Ferdig versjon lastet opp</span><a href="<?php echo esc_url($edit->edited_url); ?>" target="_blank" rel="noopener">Se ferdig bilde</a><?php else: ?><form method="post" enctype="multipart/form-data" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="9ls1_fotoportal_upload_edited_image"><input type="hidden" name="image_id" value="<?php echo (int)$edit->id; ?>"><?php wp_nonce_field('9ls1_fotoportal_upload_edited_image'); ?><input type="file" name="edited_image" accept="image/jpeg,image/png,image/webp,image/tiff" required><button class="button button-primary" type="submit">Last opp ferdig bilde</button></form><?php endif; ?></div>
                            </div>
                        <?php endforeach; ?>
                        </div>
                    </section>
                    ...
                    <section class="aurora-workspace-card">
                        <div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">DIGITAL LEVERING</span><h2>Endelig leveransesett</h2><p>Velg hva kunden skal motta. Ferdige redigerte versjoner erstatter automatisk proof/original i leveransen.</p></div><span class="aurora-status-pill <?php echo $delivery_released?'is-active':''; ?>"><?php echo $delivery_released?'Frigitt til kunde':'Ikke frigitt'; ?></span></div>
                        <div class="aurora-flow-grid" style="margin-bottom:18px">
                            <div class="<?php echo $signed?'is-complete':''; ?>"><span>1</span><strong>Kontrakt</strong><small><?php echo $signed?'Grønn ✓':'Mangler'; ?></small></div>
                            <div class="<?php echo $paid?'is-complete':''; ?>"><span>2</span><strong>Betaling</strong><small><?php echo $paid?'Grønn ✓':'Mangler'; ?></small></div>
                            <div class="<?php echo !empty($project_galleries)?'is-complete':''; ?>"><span>3</span><strong>Galleri</strong><small><?php echo !empty($project_galleries)?'Grønn ✓':'Mangler'; ?></small></div>
                            <div class="<?php echo empty($edit_state['open'])?'is-complete':''; ?>"><span>4</span><strong>Redigering</strong><small><?php echo empty($edit_state['open'])?'Grønn ✓':((int)$edit_state['done'].'/'.(int)$edit_state['total'].' ferdige'); ?></small></div>
                        </div>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            <input type="hidden" name="action" value="9ls1_fotoportal_save_delivery"><input type="hidden" name="project_id" value="<?php echo (int)$project_id; ?>"><?php wp_nonce_field('9ls1_fotoportal_save_delivery'); ?>
                            <label style="display:block;margin:0 0 10px"><input type="radio" name="delivery_mode" value="all" <?php checked(($project->delivery_mode??'all'),'all'); ?>> <strong>Lever alle ferdige bilder</strong> – hele prosjektets ferdige bildesett.</label>
                            <label style="display:block;margin:0 0 18px"><input type="radio" name="delivery_mode" value="selected" <?php checked(($project->delivery_mode??'all'),'selected'); ?>> <strong>Lever kun kundens valgte bilder</strong> – bruker bildene kunden har markert som valgt.</label>
                            <p><strong><?php echo count($delivery_images); ?> bilder</strong> er i leveransesettet med gjeldende valg.</p><div id="aurora-demo-rights" class="aurora-delivery-rights"><strong>Bruksrett for denne leveransen</strong><label><input type="radio" name="usage_rights" value="free_terms" <?php checked(($project->usage_rights??'free_terms'),'free_terms');?>> Fri bruk iht. leveringsbetingelsene</label><label><input type="radio" name="usage_rights" value="licensed" <?php checked(($project->usage_rights??'free_terms'),'licensed');?>> Bruk etter lisens / avtalte bruksrettigheter</label><textarea name="usage_rights_note" rows="2" placeholder="Eventuell beskrivelse av lisens eller bruksrett …"><?php echo esc_textarea($project->usage_rights_note??'');?></textarea></div>
                            <button class="aurora-secondary-action" type="submit">Lagre leveransevalg</button>
                            <?php if(!$delivery_released): ?><button class="aurora-primary-action" type="submit" name="release_delivery" value="1" <?php disabled(empty($delivery_flow['delivery_ready'])); ?>>Frigi levering til kunde</button><?php endif; ?>
                            <?php if(empty($delivery_flow['delivery_ready'])): ?><p style="color:#9a6b12"><strong>Leveransen er låst.</strong> Alle obligatoriske bokser må være grønne og alle redigeringsønsker ferdige.</p><?php endif; ?>
                        </form><div class="aurora-delivery-acceptance <?php echo !empty($project->delivery_terms_accepted_at)?'is-accepted':'';?>"><strong>Leveringsbetingelser</strong><span><?php echo !empty($project->delivery_terms_accepted_at)?'✓ Godkjent av kunde '.esc_html(wp_date('d.m.Y H:i',strtotime($project->delivery_terms_accepted_at))):'Venter på kundens godkjenning før HQ-nedlasting';?></span></div>
                    </section>

                    <section id="aurora-demo-payment" class="aurora-workspace-card aurora-delivery-status-card">
                        ...
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="aurora-delivery-status-form">
                            <input type="hidden" name="action" value="9ls1_fotoportal_update_payment_status"><input type="hidden" name="project_id" value="<?php echo (int)$project_id; ?>"><input type="hidden" name="aurora_workspace" value="1"><?php wp_nonce_field('9ls1_fotoportal_update_payment_status'); ?>
...
<div class="aurora-form-grid"><label>Studio / firmanavn<input name="studio_name" value="<?php echo esc_attr($ps['studio_name']); ?>"></label><label>Fotografens navn<input name="photographer_name" value="<?php echo esc_attr($ps['photographer_name']); ?>"></label><label>Konto- og innloggings-e-post<input type="email" value="<?php echo esc_attr($account->contact_email); ?>" readonly disabled><small>Administreres av Aurora Admin og brukes til innlogging og kontokommunikasjon.</small></label><label>Kunde-e-post <em>(valgfritt)</em><input type="email" name="portal_email" value="<?php echo esc_attr($ps['email']); ?>" placeholder="<?php echo esc_attr($account->contact_email); ?>"><small>Hvis tomt brukes konto-e-posten som kontaktadresse mot kunden.</small></label><label>Telefon<input name="portal_phone" value="<?php echo esc_attr($ps['phone']?:$account->contact_phone); ?>"></label><label>Nettside<input name="portal_website" value="<?php echo esc_attr($ps['website']?:($account->website_url??'')); ?>"></label><label>Profilfarge<input type="color" name="accent_color" value="<?php echo esc_attr($ps['accent_color']); ?>"></label><label class="aurora-span-2">Adresse<input name="portal_address" value="<?php echo esc_attr($ps['address']); ?>"></label><label>Postnummer<input name="portal_postal_code" value="<?php echo esc_attr($ps['address_postal_code']??''); ?>"></label><label>Sted<input name="portal_city" value="<?php echo esc_attr($ps['address_city']??''); ?>"></label><label class="aurora-span-2">Kort presentasjon<textarea name="portal_about"><?php echo esc_textarea($ps['about']); ?></textarea></label><label>Logo<?php if($ps['logo_url']):?><span class="aurora-current-upload">Valgt bilde: <?php echo esc_html(basename(parse_url($ps['logo_url'],PHP_URL_PATH)));?></span><img class="aurora-current-upload-preview" src="<?php echo esc_url($ps['logo_url']);?>" alt="Valgt logo"><?php endif;?><span class="aurora-upload-action">Velg ny fil</span><input type="file" name="portal_logo" accept="image/*"></label><label>Profilbilde<?php if($ps['profile_image_url']):?><span class="aurora-current-upload">Valgt bilde: <?php echo esc_html(basename(parse_url($ps['profile_image_url'],PHP_URL_PATH)));?></span><img class="aurora-current-upload-preview is-round" src="<?php echo esc_url($ps['profile_image_url']);?>" alt="Valgt profilbilde"><?php endif;?><span class="aurora-upload-action">Velg ny fil</span><input type="file" name="portal_profile_image" accept="image/*"></label><label class="aurora-span-2">Cover / hero-bilde<?php if($ps['cover_image_url']):?><span class="aurora-current-upload">Valgt bilde: <?php echo esc_html(basename(parse_url($ps['cover_image_url'],PHP_URL_PATH)));?></span><img class="aurora-current-upload-preview is-cover" src="<?php echo esc_url($ps['cover_image_url']);?>" alt="Valgt hero-bilde"><?php endif;?><span class="aurora-upload-action">Velg ny fil</span><input type="file" name="portal_cover_image" accept="image/*"></label></div>
...
<?php else: ?>
<section class="aurora-workspace-card aurora-profile-settings-summary aurora-settings-profile-card"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">FOTOGRAFKONTO</span><h2><?php echo esc_html($ps['studio_name']?:$account->account_name);?></h2><p><?php echo esc_html($ps['photographer_name']);?><?php if($ps['email']):?> · <?php echo esc_html($ps['email']);?><?php endif;?></p></div><div class="aurora-settings-profile-actions"><a class="aurora-primary-action aurora-settings-edit-cta" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('settings',['edit_profile'=>1]));?>"><span class="dashicons dashicons-edit"></span>Rediger profil, branding og e-post</a><a class="aurora-secondary-action" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('settings').'#vannmerke');?>"><span class="dashicons dashicons-format-image"></span>Rediger vannmerke</a></div></div><div class="aurora-settings-profile-mini"><?php if($ps['logo_url']):?><img src="<?php echo esc_url($ps['logo_url']);?>" alt="Logo"><?php endif;?><?php if($ps['profile_image_url']):?><img src="<?php echo esc_url($ps['profile_image_url']);?>" alt="Profilbilde"><?php endif;?><span style="background:<?php echo esc_attr($ps['accent_color']);?>"></span><small>Logo · profilbilde · hero · kontaktinformasjon · profilfarge · e-postmaler</small></div></section>

<?php
$module_catalog = NLS1_Aurora_Account_Platform::module_catalog();
$account_license = NLS1_Aurora_Account_Platform::get_license((int)$account->id);
?>
<section class="aurora-workspace-card aurora-subscription-modules-card">
    <div class="aurora-workspace-cardhead">
        <div>
            <span class="aurora-workspace-eyebrow">ABONNEMENT OG MODULER</span>
            <h2>Aurora Fotoportal</h2>
            <p>Se hva som er inkludert i kontoen din og hvilke tilleggsmoduler som er aktive.</p>
        </div>
        <span class="aurora-account-plan-pill"><?php echo esc_html($account->plan_name ?: ($account_license->license_name ?? 'Fotoportal')); ?></span>
    </div>
    <div class="aurora-module-overview-section">
        <strong>Inkludert i Fotoportal</strong>
        <div class="aurora-module-overview-grid">
        <?php foreach (NLS1_Aurora_Account_Platform::core_modules() as $module_key => $meta) : ?>
            <div class="aurora-module-overview-item is-active"><span class="dashicons dashicons-yes-alt"></span><div><b><?php echo esc_html($meta[0]); ?></b><small><?php echo esc_html($meta[1]); ?></small></div><em>Inkludert</em></div>
        <?php endforeach; ?>
        </div>
    </div>
    <div class="aurora-module-overview-section">
        <strong>Tilleggsmoduler</strong>
        <div class="aurora-module-overview-grid">
        <?php foreach (NLS1_Aurora_Account_Platform::addon_modules() as $module_key => $meta) : $active=!empty($enabled[$module_key]); $production_ready=!in_array($module_key,['shop','customer_app'],true); ?>
            <div class="aurora-module-overview-item <?php echo $active?'is-active':'is-locked'; ?>">
                <span class="dashicons <?php echo $active?'dashicons-yes-alt':'dashicons-lock'; ?>"></span>
                <div><b><?php echo esc_html($meta[0]); ?></b><small><?php echo esc_html($meta[1]); ?></small></div>
                <em><?php echo $active?'Aktiv':($production_ready?'Ikke aktiv':'Kommer senere'); ?></em>
            </div>
        <?php endforeach; ?>
        </div>
        <p class="aurora-module-overview-help">Standard Trial inkluderer alle ferdige Fotoportal-moduler. Nettbutikk og Customer App/PWA aktiveres først når de er produksjonsklare.</p>
    </div>
</section>

<section class="aurora-workspace-card"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">DIGITAL LEVERING</span><h2>Leveringsbetingelser og bildevisning</h2><p>Standardvilkårene vises kunden før høyoppløselige bilder kan lastes ned.</p></div></div><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="9ls1_fotoportal_save_portal_settings"><input type="hidden" name="settings_context" value="delivery"><?php wp_nonce_field('9ls1_fotoportal_save_portal_settings'); ?><div class="aurora-form-grid"><label>Versjon<input name="delivery_terms_version" value="<?php echo esc_attr($ps['delivery_terms_version']);?>"><small>Øk versjonen når vilkårene endres vesentlig.</small></label><label><span>Vis filnavn i Bildevalg</span><input type="checkbox" name="show_filenames" value="1" <?php checked(!empty($ps['show_filenames']));?>></label><label class="aurora-span-2">Standard leveringsbetingelser<textarea name="delivery_terms_text" rows="8"><?php echo esc_textarea($ps['delivery_terms_text']);?></textarea><small>Kunden må godkjenne teksten før original/HQ-nedlasting åpnes.</small></label></div><p><button class="aurora-primary-action" type="submit">Lagre leveringsinnstillinger</button></p></form></section>
...
<?php endif; ?>
<form id="vannmerke" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" class="aurora-watermark-page-form">
...
<div class="aurora-watermark-layout">
    <section class="aurora-workspace-card aurora-watermark-editor-card">
        ...
        <div class="aurora-watermark-control"><div class="aurora-range-head"><strong>4. Transparens</strong><output data-watermark-opacity-value><?php echo (int)$ps['watermark_opacity'];?> %</output></div><input type="range" min="0" max="100" name="watermark_opacity" value="<?php echo (int)$ps['watermark_opacity'];?>" data-watermark-opacity></div>
...
<?php endif; ?>
            ...
            <?php elseif ($view === 'resources') : ?>
                ...
                <?php if(($_GET['message']??'')==='resource_uploaded'):?><div class="aurora-workspace-alert is-success"><span class="dashicons dashicons-yes-alt"></span><div><strong>Ressurs lastet opp</strong><p>Filen er lagt til fotografkontoens ressurser.</p></div></div><?php elseif(($_GET['message']??'')==='demo_resource_removed'):?><div class="aurora-workspace-alert is-success"><span class="dashicons dashicons-yes-alt"></span><div><strong>Demo-innhold fjernet</strong><p>Elementet er skjult fra din konto. Aurora Admin kan gjenopprette det senere.</p></div></div><?php endif;?>
                <div class="aurora-resource-filters"><a class="<?php echo $resource_category==='all'?'is-active':'';?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('resources'));?>">Alle</a><?php foreach(['kontraktmal'=>'Kontraktmaler','dokumentmal'=>'Dokumentmaler','demo_gallery'=>'Demo-galleri','epostmal'=>'E-postmaler','shotlist'=>'Fotoplan / Shotlists','annet'=>'Andre'] as $rk=>$rl):?><a class="<?php echo $resource_category===$rk?'is-active':'';?>" href="<?php echo esc_url(NLS1_Photographer_Workspace::url('resources',['category'=>$rk]));?>"><?php echo esc_html($rl);?></a><?php endforeach;?></div>
                <div class="aurora-workspace-grid">
                    <section class="aurora-workspace-card aurora-workspace-card-wide"><div class="aurora-workspace-cardhead"><div><span class="aurora-workspace-eyebrow">RESSURSER</span><h2>Maler og filer</h2><p>Last opp kontraktmaler, guider, shotlists og andre filer du bruker i arbeidet.</p></div></div><?php if($resources):?><div class="aurora-resource-list"><?php foreach(array_reverse($resources) as $r):?><div class="aurora-resource-row"><a href="<?php echo esc_url($r['url']);?>" target="_blank" rel="noopener"><span class="dashicons <?php echo ($r['category']??'')==='demo_gallery'?'dashicons-format-image':'dashicons-media-document';?>"></span><div><strong><?php echo esc_html($r['title']);?><?php if(!empty($r['is_demo'])):?><em class="aurora-demo-badge">DEMO</em><?php endif;?></strong><small><?php echo esc_html(ucfirst(str_replace('_',' ',$r['category'])));?> · <?php echo esc_html($r['filename']);?></small></div><span class="dashicons dashicons-external"></span></a><?php if(!empty($r['is_demo'])):?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>" onsubmit="return confirm('Fjerne dette demo-elementet fra kontoen?');"><input type="hidden" name="action" value="aurora_demo_remove_resource"><input type="hidden" name="resource_id" value="<?php echo esc_attr($r['id']);?>"><?php wp_nonce_field('aurora_demo_remove_resource');?><button class="button button-link-delete" type="submit">Fjern demo</button></form><?php endif;?></div><?php endforeach;?></div><?php else:?><div class="aurora-empty-state"><strong>Ingen ressurser lastet opp ennå</strong><p>Bruk skjemaet til høyre for å legge til den første.</p></div><?php endif;?></section>
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    function sync(){ if(size){preview.style.setProperty('--wm-size',size.value+'%'); if(sizeOut)sizeOut.textContent=size.value+' %';} if(opacity){preview.style.setProperty('--wm-opacity',((100-opacity.value)/100)); if(opacityOut)opacityOut.textContent=opacity.value+' %';} }
...
<style>
.aurora-contract-gate-form{align-items:center;gap:14px;flex-wrap:wrap}.aurora-gate-save-state{font-size:12px;color:#756e7d;font-weight:600}.aurora-mail-history-list{display:grid;gap:10px;margin-top:14px}.aurora-mail-history-row{display:flex;gap:12px;align-items:flex-start;padding:14px 16px;border:1px solid #e9e4ef;border-radius:14px;background:#faf9fc}.aurora-mail-history-row.is-ok .dashicons{color:#159764}.aurora-mail-history-row.is-error{border-color:#f2c9c9;background:#fff7f7}.aurora-mail-history-row.is-error .dashicons{color:#c43838}.aurora-mail-history-row p{margin:3px 0 0;color:#716a78;font-size:12px;line-height:1.5}
</style>
