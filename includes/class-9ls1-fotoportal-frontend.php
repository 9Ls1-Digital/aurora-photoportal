...
class NLS1_Fotoportal_Frontend {
    ...
    public function __construct($register_hooks = true) {
        ...
        // even when WordPress rewrite rules are stale or the host delays a flush.
        add_filter('pre_handle_404', [$this, 'prevent_public_auth_route_404'], 0, 2);
        add_action('template_redirect', [$this, 'dispatch_public_auth_routes'], -100);
        add_action('template_redirect', [$this, 'render_photographer_customer_preview'], -10);
        add_action('template_redirect', [$this, 'render_customer_entry_login'], 0);
        ...
        add_filter('login_url', [$this, 'route_photographer_workspace_login'], 20, 3);
        ...
        add_action('wp_ajax_9ls1_accept_delivery_terms', [$this, 'handle_accept_delivery_terms']);
        add_action('admin_post_9ls1_fotoportal_customer_download', [$this, 'handle_customer_download']);
    ...
    public function add_rewrite() {
        // Public Aurora Fotoportal entry points. These routes are deliberately
        // frontend routes so photographers and customers never need wp-admin/wp-login.
        add_rewrite_rule('^fotograf/kunde/?$', 'index.php?aurora_customer_login=1', 'top');
        add_rewrite_rule('^fotograf/?$', 'index.php?aurora_photographer_login=1', 'top');
        ...
        add_rewrite_rule('^aurora/kunde/forhandsvisning/?$', 'index.php?aurora_customer_preview=1', 'top');

        // Plugin updates do not always deactivate/reactivate the plugin. Flush
        // rewrite rules once when the public route generation changes.
        $route_version = '2026-09-customer-preview-login-toggle';
        if ((string)get_option('9ls1_fotoportal_public_route_version', '') !== $route_version) {
            update_option('9ls1_fotoportal_public_route_version', $route_version, false);
            flush_rewrite_rules(false);
        }
    }

    private function public_auth_route_type() {
        $request_path = (string) wp_parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH);
        $request_path = '/' . trim(rawurldecode($request_path), '/');

        $photographer_path = (string) wp_parse_url(home_url('/fotograf/'), PHP_URL_PATH);
        $customer_path = (string) wp_parse_url(home_url('/aurora/kunde/'), PHP_URL_PATH);
        $photographer_path = '/' . trim(rawurldecode($photographer_path), '/');
        $customer_path = '/' . trim(rawurldecode($customer_path), '/');

        if ($request_path === $customer_path) return 'customer';
        if ($request_path === $photographer_path) return 'photographer';
        return '';
    }

    public function prevent_public_auth_route_404($preempt, $wp_query) {
        return $this->public_auth_route_type() ? true : $preempt;
    }

    public function dispatch_public_auth_routes() {
        $route = $this->public_auth_route_type();
        if (!$route) return;

        // Do not depend on rewrite rules for these two security-sensitive public
        // entry points. This also keeps installs working immediately after update.
        if ($route === 'customer') {
            set_query_var('aurora_customer_login', 1);
            status_header(200);
            nocache_headers();
            $this->render_customer_entry_login();
            exit;
        }

        set_query_var('aurora_photographer_login', 1);
        status_header(200);
        nocache_headers();
        $this->render_photographer_login_page();
        exit;
    }
    ...
    public function query_vars($vars) {
        ...
        $vars[] = 'aurora_customer_login';
        $vars[] = 'aurora_customer_preview';
        return $vars;
    }
    ...
    private function photographer_preview_cookie_name(){ return 'aurora_fotoportal_photographer_preview'; }
    private function set_photographer_preview_cookie($client){
        if(!$client || headers_sent())return;
        $uid=(int)get_current_user_id();$exp=time()+30*MINUTE_IN_SECONDS;
        $payload=$uid.'|'.(int)$client->id.'|'.(int)$client->account_id.'|'.$exp;
        ...
        setcookie($this->photographer_preview_cookie_name(),$value,['expires'=>$exp,'path'=>$path,'domain'=>$domain?:'','secure'=>is_ssl(),'httponly'=>true,'samesite'=>'Lax']);
        $_COOKIE[$this->photographer_preview_cookie_name()]=$value;
    }
    private function is_photographer_preview($client){
        if(!$client || !is_user_logged_in())return false;
        $user=wp_get_current_user();
        if(!($user->has_cap('aurora_fotoportal_photographer')||user_can($user,'manage_options')))return false;
        $raw=(string)($_COOKIE[$this->photographer_preview_cookie_name()]??'');if(!$raw||strpos($raw,'|')===false)return false;
        $pos=strrpos($raw,'|');$encoded=substr($raw,0,$pos);$sig=substr($raw,$pos+1);$payload=base64_decode($encoded,true);
        if($payload===false||!$sig||!hash_equals(hash_hmac('sha256',$payload,wp_salt('auth')),$sig))return false;
        $parts=explode('|',$payload);if(count($parts)!==4)return false;
        [$uid,$cid,$aid,$exp]=array_map('intval',$parts);
        if($exp<time()||$uid!==(int)$user->ID||$cid!==(int)$client->id||$aid!==(int)$client->account_id)return false;
        $user_account=(int)get_user_meta($user->ID,'aurora_fotoportal_account_id',true);
        return user_can($user,'manage_options') || ($user_account && $user_account===(int)$client->account_id);
    }
    public function render_photographer_customer_preview(){
        if(!get_query_var('aurora_customer_preview'))return;
        if(!is_user_logged_in()){wp_safe_redirect(home_url('/aurora/login/'));exit;}
        $user=wp_get_current_user();
        if(!($user->has_cap('aurora_fotoportal_photographer')||user_can($user,'manage_options'))){status_header(403);wp_die('Mangler tilgang.');}
        $client_id=absint($_GET['client_id']??0);
        if(!$client_id||!wp_verify_nonce(sanitize_text_field($_GET['_wpnonce']??''),'9ls1_customer_preview_'.$client_id)){status_header(403);wp_die('Ugyldig eller utløpt forhåndsvisning.');}
        $client=NLS1_Fotoportal_Admin::get_client($client_id);if(!$client){status_header(404);wp_die('Kunden ble ikke funnet.');}
        $this->set_photographer_preview_cookie($client);
        $gallery_id=absint($_GET['gallery_id']??0);
        if($gallery_id){$g=NLS1_Fotoportal_Admin::get_gallery($gallery_id);if($g&&(int)$g->client_id===(int)$client->id){wp_safe_redirect(NLS1_Fotoportal_Admin::gallery_public_url($g));exit;}}
        $this->render_authenticated_customer_portal($client);
    }
    ...
    public function route_photographer_workspace_login($login_url, $redirect, $force_reauth){
        $redirect = (string)$redirect;
        if ($redirect === '' || strpos($redirect, 'page=' . NLS1_Photographer_Workspace::PAGE_SLUG) === false) return $login_url;

        $parts = wp_parse_url($redirect);
        $query = [];
        if (!empty($parts['query'])) parse_str($parts['query'], $query);
        $account_id = absint($query['account_id'] ?? 0);

        // Photographer Workspace must never fall back to wp-login.php.  Old
        // bookmarks without account_id use the global Aurora photographer login;
        // account-specific bookmarks retain their tenant context.
        if (class_exists('NLS1_Aurora_Account_Platform')) {
            return NLS1_Aurora_Account_Platform::photographer_login_url($account_id);
        }
        $args = [];
    ...
    public function aurora_customer_logout_redirect($redirect_to,$requested_redirect_to,$user){
        // Customer logout must return to the shared Aurora customer login entry,
        // never to a legacy token portal. Respect an explicit Aurora redirect.
        $requested=(string)$requested_redirect_to;
        // Aurora Auth supplies the canonical destination for the active context.
        // Respect both public auth entries before applying the legacy customer-meta fallback.
        if ($requested) {
            $requested_path = (string) wp_parse_url($requested, PHP_URL_PATH);
            $photographer_path = (string) wp_parse_url(home_url('/fotograf/'), PHP_URL_PATH);
            $customer_path = (string) wp_parse_url(home_url('/aurora/kunde/'), PHP_URL_PATH);
            if ($requested_path === $photographer_path || $requested_path === $customer_path) return $requested;
        }
        if($user instanceof WP_User){
            $client_id=(int)get_user_meta($user->ID,'aurora_fotoportal_client_id',true);
            if($client_id) return home_url('/aurora/kunde/');
        }
        return $redirect_to;
    }
    ...
    private function brand_head($s,$studio,$portal='',$client=null){echo '<div class="photo-top"><div class="photo-head">';if($s['logo_url'])echo '<img class="photo-logo" src="'.esc_url($s['logo_url']).'" alt="">';elseif($s['profile_image_url'])echo '<img class="photo-avatar" src="'.esc_url($s['profile_image_url']).'" alt="">';echo '<div class="photo-brand"><strong>'.esc_html($studio).'</strong><span>'.esc_html($s['photographer_name']).'</span></div>';if($client && is_user_logged_in() && NLS1_Fotoportal_Admin::client_user_authorized($client)){ $pc=NLS1_Fotoportal_Admin::get_primary_contact((int)$client->id); $name=$pc&&$pc->contact_name?$pc->contact_name:$client->client_name; $initial=mb_strtoupper(mb_substr(trim($name),0,1)); $reset=add_query_arg(['fotoportal_password'=>1,'token'=>rawurlencode((string)$client->portal_token)],home_url('/')); $logout=function_exists('aurora_auth_logout_url')?aurora_auth_logout_url('fotoportal','customer'):wp_logout_url(home_url('/aurora/kunde/')); echo '<div class="customer-account" data-customer-account><button type="button" class="customer-account-btn" aria-expanded="false"><span class="customer-account-avatar">'.esc_html($initial).'</span><span>'.esc_html($name).'</span><span>⌄</span></button><div class="customer-account-menu"><small>'.esc_html(wp_get_current_user()->user_email).'</small><a href="'.esc_url(add_query_arg('account_view','profile',$portal?:home_url('/'))).'">Min profil</a><a href="'.esc_url(add_query_arg('account_view','status',$portal?:home_url('/'))).'">Status</a><a href="'.esc_url($portal?:home_url('/')).'">Mine prosjekter og gallerier</a><a href="'.esc_url($reset).'">Endre passord</a><hr><a class="logout" href="'.esc_url($logout).'">Logg ut</a></div></div><script>document.addEventListener("click",function(e){var w=document.querySelector("[data-customer-account]");if(!w)return;var b=w.querySelector("button");if(b.contains(e.target)){w.classList.toggle("is-open");b.setAttribute("aria-expanded",w.classList.contains("is-open")?"true":"false");}else if(!w.contains(e.target)){w.classList.remove("is-open");b.setAttribute("aria-expanded","false");}});</script>'; } elseif($portal)echo '<a class="portal-link" href="'.esc_url($portal).'">Min portal</a>';echo '</div></div>';}
    ...
    private function photographer_auth_shell($account,$title,$content){
        ...
        *{box-sizing:border-box}
        ...
        .login-card .aurora-password-field{position:relative}.login-card .aurora-password-field input{padding-right:52px!important}.login-card .aurora-password-eye{position:absolute;right:8px;top:50%;transform:translateY(-50%);width:38px;height:38px;border:0;border-radius:8px;background:transparent;color:#dbeaf0;font-size:18px;cursor:pointer}.login-card .aurora-password-eye:hover{background:rgba(255,255,255,.08)}.login-card input[type=password],.login-card input[type=text],.login-card input[type=email]{width:100%;height:52px;padding:0 15px;border:1px solid rgba(118,178,194,.45);border-radius:8px;background:rgba(0,12,24,.62);color:#fff;font:inherit;outline:none}
    ...
    public function render_photographer_password_page(){
        ...
        if($_SERVER['REQUEST_METHOD']==='POST'){
            ...
            elseif($p1!==$p2) $err='Passordene er ikke like.';
                ...
                // The permanent Test-fotograf must reproduce a brand-new activation even
                // when this password link was generated by an older build where the test
                // account was already marked onboarding=completed.
                if (!empty($account->is_test_account) && class_exists('NLS1_Aurora_Test_Photographer')) {
                    NLS1_Aurora_Test_Photographer::prepare_invitation($account_id);
                    $account=NLS1_Aurora_Account_Platform::get_account($account_id);
                }

                // Activation is the first authenticated Fotoportal session. Do not make the
                // photographer log in a second time after choosing a password.
                wp_set_current_user((int)$user->ID);
                wp_set_auth_cookie((int)$user->ID, false, is_ssl());
                do_action('wp_login', $user->user_login, $user);

                global $wpdb;
                $next_step=max(1,min(6,(int)($account->onboarding_step ?: 1)));
                if (($account->onboarding_state ?? '') !== 'completed') {
                    $wpdb->update(NLS1_Aurora_Account_Platform::table('accounts'),[
                        'onboarding_state'=>'in_progress',
                        'onboarding_step'=>$next_step,
                        'updated_at'=>current_time('mysql'),
                    ],['id'=>$account_id]);
                    wp_safe_redirect(NLS1_Photographer_Workspace::url('onboarding',['step'=>$next_step,'activated'=>1]));
                    exit;
                }
                wp_safe_redirect(NLS1_Photographer_Workspace::url('dashboard',['activated'=>1]));
                exit;
            }
        }
        ...
        ob_start(); echo $err_html.'<p>Velg passordet du vil bruke når du logger inn som fotograf.</p><form method="post" action="'.esc_url($action).'"><label>Nytt passord</label><div class="aurora-password-field"><input id="aurora-pass1" type="password" name="pass1" minlength="8" autocomplete="new-password" required><button class="aurora-password-eye" type="button" data-password-toggle="aurora-pass1" aria-label="Vis passord">👁</button></div><label>Gjenta nytt passord</label><div class="aurora-password-field"><input id="aurora-pass2" type="password" name="pass2" minlength="8" autocomplete="new-password" required><button class="aurora-password-eye" type="button" data-password-toggle="aurora-pass2" aria-label="Vis passord">👁</button></div>'; wp_nonce_field('aurora_photographer_password_reset'); echo '<button class="aurora-btn" type="submit">Lagre passord</button></form><script>document.addEventListener("click",function(e){var b=e.target.closest("[data-password-toggle]");if(!b)return;var i=document.getElementById(b.getAttribute("data-password-toggle"));if(!i)return;var show=i.type==="password";i.type=show?"text":"password";b.textContent="👁";b.setAttribute("aria-label",show?"Skjul passord":"Vis passord");});</script>'; $content=ob_get_clean();
        $this->photographer_auth_shell($account,'Opprett passord',$content);
    }

    public function render_customer_entry_login(){
        if(!get_query_var('aurora_customer_login')) return;

        if(is_user_logged_in()){
            $portal=$this->aurora_customer_portal_for_user(wp_get_current_user());
            if($portal){wp_safe_redirect($portal);exit;}
            wp_logout();
            wp_set_current_user(0);
        }
        ...
        $err='';
        $login=sanitize_text_field(wp_unslash($_REQUEST['login']??''));
        if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['aurora_global_customer_login'])){
            if(!isset($_POST['aurora_global_customer_login_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['aurora_global_customer_login_nonce'])),'aurora_global_customer_login')){
                $err='Innloggingen kunne ikke bekreftes. Prøv igjen.';
            }else{
                $creds=[
                ...
                $signed=wp_signon($creds,is_ssl());
                if(is_wp_error($signed)){
                    $err='E-post/brukernavn eller passord er ikke riktig.';
                }else{
                    $portal=$this->aurora_customer_portal_for_user($signed);
                    if(!$portal){
                        wp_logout(); wp_set_current_user(0);
                        $err='Denne innloggingen er ikke koblet til en Aurora Fotoportal-kunde.';
                    }else{
                        $client_id=(int)get_user_meta($signed->ID,'aurora_fotoportal_client_id',true);
                        $account_id=(int)get_user_meta($signed->ID,'aurora_fotoportal_account_id',true);
                        if($client_id&&$account_id){
                            $client=NLS1_Fotoportal_Admin::get_public_client_by_id_account($client_id,$account_id);
                            if($client)$this->set_customer_auth_context($client);
                        }
                        wp_set_current_user((int)$signed->ID);
                        wp_set_auth_cookie((int)$signed->ID,!empty($_POST['remember']),is_ssl());
        ...
        $branding=class_exists('NLS1_Aurora_Account_Platform') ? NLS1_Aurora_Account_Platform::platform_branding() : [];
        $bg_desktop=$branding['customer_login_bg_desktop']??($branding['photographer_login_bg_desktop']??(NLS1_FOTOPORTAL_PLUGIN_URL.'assets/aurora-login-background.png'));
        $bg_mobile=$branding['customer_login_bg_mobile']??'';
        if(!$bg_mobile)$bg_mobile=$bg_desktop;
        $logo_url=!empty($branding['logo_url'])?$branding['logo_url']:'';
        $accent=sanitize_hex_color($branding['accent_color']??'')?:'#6f4bf2';
        $action=home_url('/aurora/kunde/');

        status_header(200);nocache_headers();
        echo '<!doctype html><html lang="no"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Kundeinnlogging – Aurora Fotoportal</title><style>
        *{box-sizing:border-box}html,body{margin:0;width:100%;min-height:100%;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#fff}body{min-height:100vh;background-image:linear-gradient(180deg,rgba(25,20,28,.12),rgba(25,20,28,.34)),url("'.esc_url($bg_desktop).'");background-position:center;background-size:cover;background-repeat:no-repeat;background-attachment:fixed}.customer-entry{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:38px 18px}.customer-entry-wrap{width:min(470px,100%);text-align:center}.customer-entry-brand{margin-bottom:18px;text-shadow:0 4px 20px rgba(0,0,0,.45)}.customer-entry-brand img{max-width:220px;max-height:100px;object-fit:contain}.customer-entry-brand h2{margin:8px 0 3px;font-size:27px}.customer-entry-brand p{margin:0;color:rgba(255,255,255,.78);font-size:13px}.customer-entry-card{text-align:left;padding:28px;border:1px solid rgba(255,255,255,.28);border-radius:18px;background:rgba(34,27,38,.72);box-shadow:0 24px 70px rgba(0,0,0,.30);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}.ey{font-size:10px;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:#d8cfff}.customer-entry-card h1{margin:7px 0 8px;font-size:29px}.customer-entry-card p{color:rgba(255,255,255,.78);line-height:1.5}.customer-entry-card label{display:block;margin:14px 0 6px;font-size:12px;font-weight:750}.customer-entry-card input[type=text],.customer-entry-card input[type=password]{width:100%;height:52px;padding:0 15px;border:1px solid rgba(255,255,255,.30);border-radius:8px;background:rgba(12,8,16,.58);color:#fff;font:inherit;outline:none}.customer-entry-card input:focus{border-color:'.esc_attr($accent).';box-shadow:0 0 0 3px rgba(255,255,255,.08)}.remember{display:flex!important;align-items:center;gap:8px;font-size:13px!important;font-weight:500!important}.btn{display:flex;width:100%;min-height:50px;margin-top:16px;align-items:center;justify-content:center;border:0;border-radius:8px;background:'.esc_attr($accent).';color:#fff;font-weight:850;letter-spacing:.08em;text-transform:uppercase;cursor:pointer}.notice{padding:12px 14px;border-radius:9px;margin:14px 0;background:rgba(188,48,69,.24);border:1px solid rgba(255,111,133,.38);color:#ffe0e5;font-size:13px}.powered{margin-top:20px;color:rgba(255,255,255,.72);font-size:11px}.powered strong{display:block;margin-top:3px;color:#fff}@media(max-width:700px){body{background-image:linear-gradient(180deg,rgba(25,20,28,.14),rgba(25,20,28,.42)),url("'.esc_url($bg_mobile).'");background-attachment:scroll}.customer-entry{align-items:flex-start;padding-top:8vh}.customer-entry-card{padding:23px 20px}}</style></head><body><main class="customer-entry"><div class="customer-entry-wrap"><header class="customer-entry-brand">';
        if($logo_url)echo '<img src="'.esc_url($logo_url).'" alt="Aurora">';
        echo '<h2>Aurora Fotoportal</h2><p>Privat kundeportal</p></header><section class="customer-entry-card"><div class="ey">Kundeinnlogging</div><h1>Logg inn</h1><p>Logg inn med e-posten og passordet du har fått fra fotografen. Aurora finner riktig studio og kundeportal automatisk.</p>';
        if($err)echo '<div class="notice">'.esc_html($err).'</div>';
        echo '<form method="post" action="'.esc_url($action).'">';wp_nonce_field('aurora_global_customer_login','aurora_global_customer_login_nonce');
        echo '<input type="hidden" name="aurora_global_customer_login" value="1"><label>E-post / brukernavn</label><input type="text" name="user_login" value="'.esc_attr($login).'" autocomplete="username" required><label>Passord</label><input type="password" name="user_password" autocomplete="current-password" required><label class="remember"><input type="checkbox" name="remember" value="1"> Husk meg</label><button class="btn" type="submit">Logg inn</button></form></section><div class="powered">Powered by<strong>9Ls1 Digital</strong></div></div></main></body></html>';exit;
    }
    ...
    public function render_photographer_login_page(){
        ...
        if($account_id && !$account){status_header(404);echo '<h1>Fotografkonto finnes ikke</h1>';exit;}

        if(is_user_logged_in()){
            $current=wp_get_current_user();
            $current_account=(int)get_user_meta($current->ID,'aurora_fotoportal_account_id',true);
            $photographer=in_array('aurora_photographer',(array)$current->roles,true)||$current->has_cap('aurora_fotoportal_photographer');
            if($photographer && $current_account && (!$account_id || $current_account===$account_id)){
                NLS1_Aurora_Account_Platform::mark_account_active($current_account);
                wp_safe_redirect(NLS1_Photographer_Workspace::url('dashboard',['account_id'=>$current_account]));exit;
            }
            wp_logout();
        }
        ...
        if($_SERVER['REQUEST_METHOD']==='POST'){
            ...
            if(is_wp_error($signed)) $err='E-post/brukernavn eller passord er ikke riktig.';
                ...
                $signed_account=(int)get_user_meta($signed->ID,'aurora_fotoportal_account_id',true);
                $photographer=in_array('aurora_photographer',(array)$signed->roles,true)||$signed->has_cap('aurora_fotoportal_photographer');
                $belongs=$signed_account>0 && (!$account_id || $signed_account===$account_id);
                if(!$belongs||!$photographer){wp_logout();$err='Denne innloggingen har ikke tilgang til denne fotografkontoen.';}
                else {
                    if (!$signed->has_cap('manage_options') && !$signed->has_cap('manage_woocommerce') && !in_array('aurora_photographer',(array)$signed->roles,true)) {
                        $signed->set_role('aurora_photographer');
                    }
                    NLS1_Aurora_Account_Platform::mark_account_active($signed_account);
                    wp_safe_redirect(NLS1_Photographer_Workspace::url('dashboard',['account_id'=>$signed_account]));exit;
                }
            }
        }
        $err_html=$err?'<div class="notice err">'.esc_html($err).'</div>':'';
        $args=[]; if($account_id)$args['account_id']=$account_id;
        $action=class_exists('NLS1_Aurora_Account_Platform') ? NLS1_Aurora_Account_Platform::photographer_login_url($account_id) : add_query_arg($args,home_url('/fotograf/'));
        $intro=$account ? 'Logg inn for å åpne ditt fotograf-Workspace.' : 'Logg inn med fotografkontoen din. Aurora finner automatisk riktig studio og Workspace.';
        ob_start(); echo $err_html.'<p>'.esc_html($intro).'</p><form method="post" action="'.esc_url($action).'"><label>E-post / brukernavn</label><input type="text" name="user_login" value="'.esc_attr($login).'" autocomplete="username" required><label>Passord</label><input type="password" name="user_password" autocomplete="current-password" required><label style="font-weight:400"><input type="checkbox" name="remember" value="1"> Husk meg</label>'; wp_nonce_field('aurora_photographer_login'); echo '<button class="aurora-btn" type="submit">Logg inn</button></form>'; $content=ob_get_clean();
    ...
    public function render_customer_password_page(){
        ...
        $portal_url=NLS1_Fotoportal_Admin::customer_portal_url((int)$client->id,(int)$client->account_id);
        $email=NLS1_Fotoportal_Admin::client_portal_email((int)$client->id,(int)$client->account_id);
        $user=$email?get_user_by('email',$email):false;
        // Self-heal legacy customers: the contact email is the source of truth for the portal login.
        if(!$user && $email){$uid=NLS1_Fotoportal_Admin::ensure_client_portal_user((int)$client->id,(int)$client->account_id);if($uid)$user=get_user_by('id',$uid);}
        elseif($user){NLS1_Fotoportal_Admin::ensure_client_portal_user((int)$client->id,(int)$client->account_id);}
        ...
        if($mode==='reset'){
            ...
            if($_SERVER['REQUEST_METHOD']==='POST'){
                ...
                $p1=(string)($_POST['pass1']??''); $p2=(string)($_POST['pass2']??'');
                if(strlen($p1)<8){$err='Passordet må inneholde minst 8 tegn.';} elseif($p1!==$p2){$err='Passordene er ikke like.';} else {
                    reset_password($reset_user,$p1);
                    // A successful password reset is already a strong proof that the
                    // customer controls the reset mailbox. Finish the flow in one step:
                    // repair the customer mapping, create the WP auth session and enter
                    // the portal directly. This also avoids reusing the now-consumed
                    // reset key and removes the confusing second-login round trip.
                    if(!NLS1_Fotoportal_Admin::repair_client_user_authorization($client,(int)$reset_user->ID)){
                        $content='<div class="notice err"><strong>Passordet ble lagret, men kundeportalen kunne ikke kobles til brukeren.</strong></div><p>Kontakt fotografen for å reparere kundeinnloggingen.</p>';
                        $this->customer_password_page_shell($client,'Kundeinnlogging må repareres',$content);
                    }
                    wp_set_current_user((int)$reset_user->ID);
                    wp_set_auth_cookie((int)$reset_user->ID,true,is_ssl());
                    $this->set_customer_auth_context($client);
                    do_action('wp_login',$reset_user->user_login,$reset_user);
                    $destination=NLS1_Fotoportal_Admin::customer_portal_url((int)$client->id,(int)$client->account_id);
                    if(!$destination){
                        $destination=add_query_arg(['fotoportal_customer'=>1,'token'=>rawurlencode((string)$client->portal_token)],home_url('/'));
                    }
    ...
    private function tracked_delivery_url($client,$project_id,$kind,$source_url,$image_id=0,$photographer_preview=false){
        if($photographer_preview || !$client || !$source_url)return $source_url;
        $args=['action'=>'9ls1_fotoportal_customer_download','project_id'=>(int)$project_id,'client_id'=>(int)$client->id,'kind'=>sanitize_key($kind),'_wpnonce'=>wp_create_nonce('9ls1_customer_download_'.(int)$project_id.'_'.(int)$client->id)];
        if($image_id)$args['image_id']=(int)$image_id;
        return add_query_arg($args,admin_url('admin-post.php'));
    }
    private function local_path_from_upload_url($url){
        $u=wp_upload_dir();
        $baseurl=rtrim((string)$u['baseurl'],'/');
        if(strpos((string)$url,$baseurl)!==0)return '';
        $rel=ltrim(substr((string)$url,strlen($baseurl)),'/');
        $path=trailingslashit($u['basedir']).$rel;
        $real=realpath($path);$base=realpath($u['basedir']);
        return ($real&&$base&&strpos($real,$base)===0&&is_file($real))?$real:'';
    }
    public function handle_customer_download(){
        $project_id=absint($_GET['project_id']??0);$client_id=absint($_GET['client_id']??0);$kind=sanitize_key($_GET['kind']??'hq_zip');$image_id=absint($_GET['image_id']??0);
        if(!$project_id||!$client_id||!wp_verify_nonce(sanitize_text_field($_GET['_wpnonce']??''),'9ls1_customer_download_'.$project_id.'_'.$client_id))wp_die('Ugyldig eller utløpt nedlastingslenke.');
        $c=NLS1_Fotoportal_Admin::get_public_client_by_id_account($client_id,(int)get_user_meta(get_current_user_id(),'aurora_fotoportal_account_id',true));
        if(!$c||!NLS1_Fotoportal_Admin::client_user_authorized($c))wp_die('Ingen tilgang.');
        $del=NLS1_Fotoportal_Admin::customer_delivery_for_project($project_id,(int)$c->account_id);if(!$del||empty($del['project']->delivery_terms_accepted_at))wp_die('Leveransen er ikke klar for nedlasting.');
        $url='';$filename='';
        if($kind==='hq_zip'){$url=(string)($del['zip_url']??'');$filename='bilder-'.$project_id.'.zip';}
        elseif($kind==='hq_selected_zip'){$url=(string)($del['selected_zip_url']??'');$filename='valgte-bilder-'.$project_id.'.zip';}
        elseif($kind==='hq_image'&&$image_id){foreach($del['images'] as $im){if((int)$im->id===$image_id){$edited=(($im->edit_status??'none')==='ready'&&!empty($im->edited_url));$url=$edited?(string)$im->edited_url:(string)$im->original_url;$filename=$edited?($im->edited_filename?:$im->original_filename):$im->original_filename;break;}}}
        $path=$this->local_path_from_upload_url($url);if(!$path)wp_die('Filen kunne ikke finnes.');
        NLS1_Fotoportal_Admin::record_customer_download($project_id,$client_id,$kind,$image_id);
        nocache_headers();header('Content-Type: application/octet-stream');header('Content-Disposition: attachment; filename="'.sanitize_file_name($filename?:basename($path)).'"');header('Content-Length: '.filesize($path));readfile($path);exit;
    }

    public function handle_accept_delivery_terms(){
        check_ajax_referer('9ls1_delivery_terms','nonce'); global $wpdb;
        $project_id=absint($_POST['project_id']??0); $client_id=absint($_POST['client_id']??0);
        $account_id=(int)get_user_meta(get_current_user_id(),'aurora_fotoportal_account_id',true); $c=NLS1_Fotoportal_Admin::get_public_client_by_id_account($client_id,$account_id);
        if(!$c||!NLS1_Fotoportal_Admin::client_user_authorized($c))wp_send_json_error(['message'=>'Ingen tilgang'],403);
        $p=$wpdb->get_row($wpdb->prepare("SELECT * FROM ".NLS1_Fotoportal_Admin::table('projects')." WHERE id=%d AND client_id=%d AND account_id=%d",$project_id,$client_id,$account_id));
    ...
    public function render_customer_portal(){if(!get_query_var('fotoportal_customer'))return;$c=NLS1_Fotoportal_Admin::get_public_client_by_token(sanitize_text_field($_GET['token']??''));status_header($c?200:404);nocache_headers();if(!$c){echo '<h1>Portalen ble ikke funnet</h1>';exit;}if(class_exists('NLS1_Aurora_Account_Platform')&&!NLS1_Aurora_Account_Platform::is_module_enabled((int)$c->account_id,'customer_portal')){status_header(403);echo '<h1>Kundeportal er ikke aktivert</h1><p>Fotografen har ikke aktivert Kundeportal for denne kontoen.</p>';exit;}$demo_preview=class_exists('NLS1_Aurora_Demo_Journey')&&NLS1_Aurora_Demo_Journey::is_valid_preview($c);$photographer_preview=$this->is_photographer_preview($c);if(!$demo_preview&&!$photographer_preview)$this->set_customer_auth_context($c);$return_url=home_url(add_query_arg([],$_SERVER['REQUEST_URI']??'/'));if(!$demo_preview&&!$photographer_preview&&(!is_user_logged_in()||!NLS1_Fotoportal_Admin::client_user_authorized($c))){$this->render_customer_login_gate($c,$return_url);}$s=NLS1_Fotoportal_Admin::photographer_portal_settings($c->account_id);$gs=NLS1_Fotoportal_Admin::get_public_client_projects_and_galleries($c);$studio=$s['studio_name']?:($s['photographer_name']?:get_bloginfo('name'));$a=sanitize_hex_color($s['accent_color'])?:'#6f4bf2';echo '<!doctype html><html lang="no"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.esc_html($studio).' – Kundeportal</title><style>:root{--a:'.$a.'}'.$this->brand_css().'.shell{max-width:1240px;margin:auto;padding:34px 20px 55px}.welcome h1{font-size:34px;margin:0 0 8px}.welcome p{color:#817889}.project{margin:30px 0}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.card{background:#fff;border:1px solid #e9e4ed;border-radius:14px;overflow:hidden;text-decoration:none;color:inherit}.cover{aspect-ratio:16/10;background:#e9e6ec}.cover img{width:100%;height:100%;object-fit:cover}.cb{padding:15px}.cb strong{display:block}.cb p{color:#817889}.open{color:var(--a);font-weight:700}.final-delivery{background:#fff;border:1px solid #e9e4ed;border-radius:16px;padding:24px;margin:26px 0}.final-delivery-head{display:flex;justify-content:space-between;gap:18px;align-items:center}.final-delivery-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:18px}.final-delivery-image{border:1px solid #eee7f0;border-radius:12px;overflow:hidden}.final-delivery-image img{width:100%;aspect-ratio:1/1;object-fit:cover;display:block}.final-delivery-image div{padding:10px}.download-btn{display:inline-block;background:var(--a);color:#fff!important;text-decoration:none;padding:10px 15px;border-radius:9px;font-weight:700}.final-delivery.is-locked a[download],.final-delivery.is-locked a.download-btn,.final-delivery.is-locked a.open{display:none!important}.delivery-terms{border:1px solid #e9e4ed;background:#faf9fb;border-radius:12px;padding:18px;margin-bottom:20px}.edited-badge{display:inline-block;background:#eaf8f0;color:#157849;padding:4px 8px;border-radius:999px;font-size:12px;font-weight:700}.gallery-state{display:inline-flex;align-items:center;gap:6px;margin-top:10px;padding:6px 10px;border-radius:999px;font-size:12px;font-weight:800;background:#f2eefc;color:#6540d8}.gallery-state.is-ready{background:#eaf8f0;color:#157849}.status-journey{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:16px}.status-step{border:1px solid #ebe5ef;border-radius:14px;padding:15px;background:#faf9fb}.status-step.done{background:#f0faf5;border-color:#ccebd9}.status-step b{display:block;margin-top:5px}.status-progress{height:8px;background:#eee9f2;border-radius:999px;overflow:hidden;margin:14px 0}.status-progress i{display:block;height:100%;background:var(--a);border-radius:999px}@media(max-width:800px){.status-journey{grid-template-columns:1fr 1fr}}@media(max-width:800px){.cards{grid-template-columns:1fr 1fr}.final-delivery-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:520px){.cards,.final-delivery-grid{grid-template-columns:1fr}.final-delivery-head{display:block}}</style></head><body>';$this->brand_head($s,$studio,NLS1_Fotoportal_Admin::customer_portal_url((int)$c->id),$c);if(!empty($demo_preview)){echo '<div style="position:sticky;top:0;z-index:9999;background:#171320;color:#fff;padding:10px 18px;display:flex;justify-content:space-between;align-items:center;gap:12px;font:600 13px/1.3 system-ui"><span><b style="color:#bfa8ff">DEMO</b> – Du ser portalen som kunde. Fotograf-sessionen er fortsatt aktiv.</span><a style="color:#fff;text-decoration:none;border:1px solid rgba(255,255,255,.25);border-radius:8px;padding:7px 10px" href="'.esc_url(NLS1_Photographer_Workspace::url('dashboard',['demo_journey'=>1])).'">← Tilbake til Demo Guide</a></div>';}if(!empty($photographer_preview)){echo '<div style="position:sticky;top:0;z-index:9999;background:#171320;color:#fff;padding:10px 18px;display:flex;justify-content:space-between;align-items:center;gap:12px;font:600 13px/1.3 system-ui"><span><b style="color:#70e0ca">FORHÅNDSVISNING</b> – Du ser kundeportalen som kunden. Kundens egen innlogging påvirkes ikke.</span><a style="color:#fff;text-decoration:none;border:1px solid rgba(255,255,255,.25);border-radius:8px;padding:7px 10px" href="'.esc_url(NLS1_Photographer_Workspace::url('customers',['customer_id'=>(int)$c->id])).'">← Tilbake til kunden</a></div>';}$chs=NLS1_Fotoportal_Admin::public_customer_hero_settings($c);$chi=[];foreach($gs as $xx)foreach($xx['galleries'] as $gg){$gi=NLS1_Fotoportal_Admin::get_public_gallery_images($gg);$chi=array_merge($chi,$gi);}$hero=NLS1_Fotoportal_Admin::hero_image_url($chs,$chi,$s['cover_image_url']??'');echo '<section class="photo-hero size-'.esc_attr($chs['size']).' '.($hero?'':'no-image').'"'.($hero?' style="background-image:url('.esc_url($hero).');background-position:'.(int)$chs['focal_x'].'% '.(int)$chs['focal_y'].'%"':'').'><span class="photo-hero-overlay" style="background:'.esc_attr($chs['overlay_color']).';opacity:'.esc_attr((100-$chs['overlay_opacity'])/100).'"></span><div class="photo-hero-content"><div class="hero-kicker">'.esc_html($studio).'</div><h1>'.esc_html($c->client_name).'</h1><p>Velkommen til din bildeportal</p></div></section><main class="shell">'; $pc=NLS1_Fotoportal_Admin::get_primary_contact((int)$c->id); $all_signed=true;$all_paid=true;$has_gallery=false; foreach($gs as $sx){$st=NLS1_Fotoportal_Admin::public_project_delivery_state((int)$sx['project']->project_id,(int)$c->account_id);if(empty($st['contract_signed']))$all_signed=false;if(empty($st['paid']))$all_paid=false;if(!empty($st['gallery']))$has_gallery=true;} $account_view=sanitize_key($_GET['account_view']??'');
        if($account_view==='profile'){
            ...
            echo '<section class="account-view" style="background:#fff;border:1px solid #e9e4ed;border-radius:18px;padding:30px;margin:0 0 28px"><a class="open" href="'.esc_url(NLS1_Fotoportal_Admin::customer_portal_url((int)$c->id)).'">← Tilbake til portalen</a><div style="display:flex;justify-content:space-between;gap:18px;align-items:end;flex-wrap:wrap"><div><small style="font-weight:800;color:var(--a);letter-spacing:.08em">MIN FOTOREISE</small><h2 style="font-size:30px;margin:5px 0 4px">Status</h2><p style="color:#817889;margin:0">Følg prosjektet fra avtale til ferdige bilder.</p></div></div>';
            $status_projects=NLS1_Fotoportal_Admin::get_public_client_project_statuses($c);
            foreach($status_projects as $sp){
                $st=NLS1_Fotoportal_Admin::public_project_delivery_state((int)$sp->project_id,(int)$c->account_id); $dl=NLS1_Fotoportal_Admin::project_download_stats((int)$sp->project_id,(int)$c->account_id);
                $steps=[['Kontrakt',!empty($st['contract_signed']),'Signert','Venter på signering'],['Betaling',!empty($st['paid']),'Betalt','Venter'],['Galleri',!empty($st['gallery']),((int)$st['gallery_count']).' galleri'.((int)$st['gallery_count']===1?'':'er').' klart','Venter'],['Leveranse',!empty($st['portal_ready']),(!empty($dl['count'])?'Lastet ned':'Klar'),'Venter']];
                $done=0;foreach($steps as $z)if($z[1])$done++;$pct=(int)round(($done/4)*100);
                echo '<div style="border-top:1px solid #eee7f0;padding:24px 0 6px;margin-top:22px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center"><h3 style="margin:0">'.esc_html($sp->project_name).'</h3><strong style="color:var(--a)">'.$pct.' %</strong></div><div class="status-progress"><i style="width:'.$pct.'%"></i></div><div class="status-journey">';
                $n=0;foreach($steps as $it){$n++;echo '<div class="status-step '.($it[1]?'done':'').'"><small>STEG '.$n.'</small><strong>'.esc_html($it[0]).'</strong><b>'.esc_html($it[1]?('✓ '.$it[2]):$it[3]).'</b></div>';}
                echo '</div>'; if(!empty($dl['count']))echo '<p style="margin:14px 0 0;color:#157849;font-weight:700">✓ Bilder lastet ned '.(int)$dl['count'].' gang'.((int)$dl['count']===1?'':'er').(!empty($dl['last_at'])?' · sist '.esc_html(mysql2date('d.m.Y H:i',$dl['last_at'])):'').'</p>'; echo '</div>';
            }
            echo '</section>';
        }
        echo '<div id="aurora-prosjekter"></div>';foreach($gs as $x){echo '<section class="project"><h2>'.esc_html($x['project']->project_name).'</h2><div class="cards">';foreach($x['galleries'] as $g){$ic=NLS1_Fotoportal_Admin::gallery_interaction_counts((int)$g->gallery_id,(int)$g->account_id);$gst=NLS1_Fotoportal_Admin::public_project_delivery_state((int)$x['project']->project_id,(int)$c->account_id);$gdel=NLS1_Fotoportal_Admin::customer_delivery_for_project((int)$x['project']->project_id,(int)$c->account_id);$gready=$gdel&&!empty($gdel['project']->delivery_terms_accepted_at);$greleased=$gdel&&!empty($gdel['project']->delivery_released_at);$glabel=$gready?'Klar for nedlasting':($greleased?'Leveranse klar – godkjenn vilkår':(!empty($gst['gallery'])?'Galleri tilgjengelig':'Venter'));$interactions=!class_exists('NLS1_Aurora_Account_Platform')||NLS1_Aurora_Account_Platform::is_module_enabled((int)$g->account_id,'favorites_comments');$gallery_href=$g->public_url;if(!empty($demo_preview))$gallery_href=add_query_arg(['aurora_demo_preview'=>1,'_aurora_demo_nonce'=>sanitize_text_field($_GET['_aurora_demo_nonce']??'')],$gallery_href);echo '<a class="card" href="'.esc_url($gallery_href).'"><div class="cover">'.($g->cover_url?'<img src="'.esc_url($g->cover_url).'" alt="">':'').'</div><div class="cb"><strong>'.esc_html($g->gallery_title).'</strong><p>'.esc_html(!empty($g->gallery_description)?$g->gallery_description:((int)$g->original_count.' bilder')).'</p><div class="interaction-strip"><span>♡ <b>'.(int)$ic['favorites'].'</b></span><span>✓ <b>'.(int)$ic['approved'].'</b> valgt</span><span>💬 <b>'.(int)$ic['comments'].'</b></span></div>'.NLS1_Fotoportal_Admin::public_selection_status_badge($g).'<span class="gallery-state '.($gready?'is-ready':'').'">'.($gready?'✓ ':'').esc_html($glabel).'</span><p class="open">Åpne galleri →</p></div></a>';}echo '</div>';
            $digital=!class_exists('NLS1_Aurora_Account_Platform')||NLS1_Aurora_Account_Platform::is_module_enabled((int)$c->account_id,'hq_delivery');
            if($digital){$del=NLS1_Fotoportal_Admin::customer_delivery_for_project((int)$x['project']->project_id,(int)$c->account_id);if($del){$accepted=!empty($del['project']->delivery_terms_accepted_at);$rights=(($del['project']->usage_rights??'free_terms')==='licensed')?'Bruk etter lisens / avtalte bruksrettigheter':'Fri bruk iht. leveringsbetingelsene';echo '<div class="final-delivery '.($accepted?'is-accepted':'is-locked').'">';if(!$accepted){echo '<div class="delivery-terms"><h3>Godkjenn leveringsbetingelser</h3><div style="white-space:pre-wrap;color:#625a66">'.esc_html($s['delivery_terms_text']??'').'</div><p><strong>Bruksrett:</strong> '.esc_html($rights).'</p>'.(!empty($del['project']->usage_rights_note)?'<p>'.esc_html($del['project']->usage_rights_note).'</p>':'').'<label style="display:flex;gap:9px;align-items:flex-start;margin:16px 0"><input type="checkbox" data-terms-check> <span>Jeg har lest og godtar leveringsbetingelsene.</span></label><button class="download-btn" type="button" data-accept-terms data-project="'.(int)$x['project']->project_id.'" data-client="'.(int)$c->id.'" disabled>Godkjenn og åpne nedlasting</button></div>';};echo '<div class="final-delivery-head"><div class="final-delivery-head"><div><small style="color:#817889;font-weight:700">DIGITAL LEVERING</small><h3 style="margin:4px 0">Dine ferdige bilder er klare</h3><p style="color:#817889">Dette er fotografens endelige leveransesett. Redigerte bilder etter ønske erstatter automatisk tidligere proof-versjon.</p></div>'.'<div style="display:flex;gap:8px;flex-wrap:wrap">'.(!empty($del['zip_url'])?'<a class="download-btn" href="'.esc_url($this->tracked_delivery_url($c,(int)$x['project']->project_id,'hq_zip',$del['zip_url'],0,$photographer_preview)).'">Last ned alle bilder</a>':'').(!empty($del['selected_zip_url'])?'<a class="download-btn" href="'.esc_url($this->tracked_delivery_url($c,(int)$x['project']->project_id,'hq_selected_zip',$del['selected_zip_url'],0,$photographer_preview)).'">Last ned valgte bilder</a>':'').'</div>'.'</div>'; $edited_items=array_values(array_filter($del['images'],function($im){return (($im->edit_status??'none')==='ready'&&!empty($im->edited_url));})); if($edited_items){echo '<div style="margin-top:22px"><h4 style="margin-bottom:6px">Redigerte bilder etter ønske</h4><p style="color:#817889;margin-top:0">Ferdige versjoner av bilder du ba fotografen redigere.</p><div class="final-delivery-grid">';foreach($edited_items as $ei){echo '<div class="final-delivery-image"><img src="'.esc_url($ei->edited_url).'" alt=""><div><span class="edited-badge">Redigert etter ønske</span><p>'.esc_html($ei->edited_filename?:$ei->original_filename).'</p><a class="open" href="'.esc_url($this->tracked_delivery_url($c,(int)$x['project']->project_id,'hq_image',$ei->edited_url,(int)$ei->id,$photographer_preview)).'">Last ned bilde</a></div></div>';}echo '</div></div>';} echo '<h4 style="margin:24px 0 8px">Hele leveransen</h4><div class="final-delivery-grid">';foreach($del['images'] as $di){$edited=(($di->edit_status??'none')==='ready'&&!empty($di->edited_url));$url=$edited?$di->edited_url:$di->original_url;$preview=$edited?$di->edited_url:($di->thumbnail_url?:$di->preview_url);if(!$url)continue;echo '<div class="final-delivery-image"><img src="'.esc_url($preview?:$url).'" alt=""><div>'.($edited?'<span class="edited-badge">Redigert etter ønske</span>':'').'<p style="word-break:break-word">'.esc_html($edited?($di->edited_filename?:$di->original_filename):$di->original_filename).'</p><a class="open" href="'.esc_url($this->tracked_delivery_url($c,(int)$x['project']->project_id,'hq_image',$url,(int)$di->id,$photographer_preview)).'">Last ned bilde</a></div></div>';}echo '</div></div>';}}
            echo '</section>';}echo '</main><script>(function(){document.querySelectorAll("[data-accept-terms]").forEach(function(btn){var box=btn.closest(".delivery-terms"),chk=box&&box.querySelector("[data-terms-check]");if(!chk)return;chk.onchange=function(){btn.disabled=!chk.checked};btn.onclick=function(){var f=new FormData();f.append("action","9ls1_accept_delivery_terms");f.append("nonce","'.wp_create_nonce('9ls1_delivery_terms').'");f.append("project_id",btn.dataset.project);f.append("client_id",btn.dataset.client);fetch("'.esc_url(admin_url('admin-ajax.php')).'",{method:"POST",credentials:"same-origin",body:f}).then(function(r){return r.json()}).then(function(j){if(j.success)location.reload();else alert((j.data&&j.data.message)||"Kunne ikke lagre godkjenningen")})}})})();</script>';$this->brand_foot($s,$studio);echo '</body></html>';exit;}
    private function gallery_interaction_state($g,$visitor=''){
        ...
        $state['counts']['comments']=(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $com WHERE gallery_id=%d AND deleted_at IS NULL",$g->id));
        ...
        $rows=$wpdb->get_results($wpdb->prepare("SELECT id,image_id,comment_text,created_at,author_type,author_user_id,user_email,is_edit_request FROM $com WHERE gallery_id=%d AND deleted_at IS NULL ORDER BY created_at ASC,id ASC",$g->id));
        foreach($rows as $r){$id=(int)$r->image_id;if(!isset($state['comments'][$id]))$state['comments'][$id]=[];$u=wp_get_current_user();$mine=(($r->author_type??'customer')==='customer' && ((int)$r->author_user_id===0 || (int)$r->author_user_id===(int)$u->ID));$state['comments'][$id][]=['id'=>(int)$r->id,'text'=>$r->comment_text,'date'=>$r->created_at,'author_type'=>$r->author_type?:'customer','author_label'=>(($r->author_type??'customer')==='photographer'?'Fotograf':'Kunde'),'can_delete'=>$mine,'is_edit_request'=>!empty($r->is_edit_request)];}
        return $state;
    }
    ...
    public function handle_gallery_interaction(){
        ...
        if(class_exists('NLS1_Aurora_Account_Platform')&&!NLS1_Aurora_Account_Platform::is_module_enabled((int)$g->account_id,'favorites_comments'))wp_send_json_error(['message'=>'Favoritter og kommentarer er ikke aktivert for denne fotografkontoen.'],403);
        ...
        if($kind==='favorite'){
            ...
            $u=wp_get_current_user();$wpdb->insert(self::safe_table('image_comments'),['image_id'=>$image_id,'gallery_id'=>$g->id,'project_id'=>$g->project_id,'client_id'=>$g->client_id,'user_email'=>(string)$u->user_email,'author_type'=>'customer','author_user_id'=>(int)$u->ID,'is_edit_request'=>!empty($_POST['is_edit_request'])?1:0,'comment_text'=>$text,'is_test'=>0,'created_at'=>current_time('mysql')]);$active=true;
        }elseif($kind==='delete_comment'){
            $comment_id=absint($_POST['comment_id']??0);$t=self::safe_table('image_comments');$u=wp_get_current_user();
            $row=$wpdb->get_row($wpdb->prepare("SELECT * FROM $t WHERE id=%d AND image_id=%d AND gallery_id=%d AND deleted_at IS NULL",$comment_id,$image_id,$g->id));
            if(!$row||($row->author_type??'customer')!=='customer')wp_send_json_error(['message'=>'Kommentaren kan ikke slettes'],403);
            if((int)$row->author_user_id && (int)$row->author_user_id!==(int)$u->ID)wp_send_json_error(['message'=>'Du kan bare slette egne kommentarer'],403);
            $wpdb->update($t,['deleted_at'=>current_time('mysql')],['id'=>$comment_id]);$active=false;
    ...
    public function render_public_gallery(){
        ...
        $token=sanitize_text_field($_GET['token']??'');$g=NLS1_Fotoportal_Admin::get_public_gallery_by_token($token);status_header($g?200:404);nocache_headers();if(!$g){echo '<h1>Galleriet ble ikke funnet</h1>';exit;}$c=NLS1_Fotoportal_Admin::get_public_client_by_id_account((int)$g->client_id,(int)$g->account_id);if(!$c){status_header(404);echo '<h1>Galleriet ble ikke funnet</h1>';exit;}$demo_preview=class_exists('NLS1_Aurora_Demo_Journey')&&NLS1_Aurora_Demo_Journey::is_valid_preview($c);$photographer_preview=$this->is_photographer_preview($c);if(!$demo_preview&&!$photographer_preview)$this->set_customer_auth_context($c);$return_url=home_url(add_query_arg([],$_SERVER['REQUEST_URI']??'/'));if(!$demo_preview&&!$photographer_preview&&(!is_user_logged_in()||!NLS1_Fotoportal_Admin::client_user_authorized($c))){$this->render_customer_login_gate($c,$return_url);}if(!NLS1_Fotoportal_Admin::public_project_portal_ready((int)$g->project_id,(int)$g->account_id)){status_header(403);echo '<h1>Galleriet er ikke tilgjengelig</h1><p>Prosjektet er ikke frigitt for levering.</p>';exit;}
        $interactions_enabled=!class_exists('NLS1_Aurora_Account_Platform')||NLS1_Aurora_Account_Platform::is_module_enabled((int)$g->account_id,'favorites_comments');
        ...
        echo '<section class="photo-hero size-'.esc_attr($ghs['size']).' '.($galleryHero?'':'no-image').'"'.($galleryHero?' style="background-image:url('.esc_url($galleryHero).');background-position:'.(int)$ghs['focal_x'].'% '.(int)$ghs['focal_y'].'%"':'').'><span class="photo-hero-overlay" style="background:'.esc_attr($ghs['overlay_color']).';opacity:'.esc_attr((100-$ghs['overlay_opacity'])/100).'"></span><div class="photo-hero-content"><div class="hero-kicker">'.esc_html($g->project_name).'</div><h1>'.esc_html($g->gallery_title).'</h1><p>'.esc_html(!empty($g->gallery_description)?$g->gallery_description:(count($ims).' bilder')).'</p></div></section><main class="shell"><header class="gh"><div><div class="ey">'.esc_html($studio).'</div><h2>'.esc_html($g->gallery_title).'</h2>';if($interactions_enabled){echo '<div class="interaction-strip" data-gallery-filters><button type="button" class="interaction-filter is-active" data-filter="all">Alle bilder</button><button type="button" class="interaction-filter" data-filter="favorites">♡ <b data-count="favorites">'.$state['counts']['favorites'].'</b> favoritter</button><button type="button" class="interaction-filter" data-filter="approved">✓ <b data-count="approved">'.$state['counts']['approved'].'</b> valgt</button><button type="button" class="interaction-filter" data-filter="comments">💬 <b data-count="comments">'.$state['counts']['comments'].'</b> kommentarer</button></div>';}echo '</div><div class="meta">'.count($ims).' bilder</div></header>';echo '<section class="masonry">';
        foreach($ims as $im){$src=$im->preview_url?:$im->thumbnail_url;if(!$src)continue;$approved=!empty($im->is_selected);echo '<figure class="item" data-image-id="'.(int)$im->id.'"><img loading="lazy" src="'.esc_url($src).'" data-full="'.esc_url($src).'" alt="">';if($interactions_enabled)echo '<div class="image-actions"><button type="button" class="image-action favorite" title="Favoritt" aria-label="Favoritt">♡</button><button type="button" class="image-action approve'.($approved?' is-active':'').'" title="Godkjenn / velg" aria-label="Godkjenn / velg">✓</button><button type="button" class="image-action comment" title="Kommentar til ønsket redigering" aria-label="Kommentar til ønsket redigering">💬</button></div><div class="comment-box"><button type="button" class="comment-box-close" aria-label="Lukk">×</button><div class="comment-list"></div><textarea placeholder="Skriv en kommentar …"></textarea><label style="display:flex;gap:6px;align-items:center;font-size:11px;margin-top:6px"><input type="checkbox" class="edit-request-check" checked> Dette er et redigeringsønske</label><button type="button" class="comment-add">Legg til kommentar</button></div>';echo '</figure>';}
    ...
    public function handle_submit_gallery_selection(){
        ...
        if((int)$counts['approved']<1) wp_send_json_error(['message'=>'Velg minst ett bilde før du sender valget til fotografen.'],400);
    ...
    public function handle_sign_contract() {
        ...
        if ($project) {
            ...
            NLS1_Fotoportal_Admin::notify_photographer_contract_signed((int)$project->id,$signer_name,$signer_email,current_time('mysql'));
