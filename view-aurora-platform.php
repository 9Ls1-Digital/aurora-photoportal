                    ...
                    <table class="widefat striped aurora-account-registry-table">
                        ...
                        <?php foreach ($accounts as $row) : ?>
                            <tr class="<?php echo $account && (int)$account->id===(int)$row->id?'is-selected':''; ?>">
                                <td><strong><a href="<?php echo esc_url(NLS1_Aurora_Account_Platform::url('accounts',['account_id'=>$row->id])); ?>"><?php echo esc_html($row->account_name); ?></a></strong><small><?php echo esc_html($row->billing_city ?: $row->account_slug); ?></small></td>
